...
def run_once(binary: Path, url: str, expression: str, expected: int) -> tuple[float, int]:
    with tempfile.NamedTemporaryFile(prefix="telemaco-rss-", delete=False) as rss_file:
        rss_path = Path(rss_file.name)
    with tempfile.NamedTemporaryFile(prefix="telemaco-stdout-", delete=False) as stdout_file:
        stdout_path = Path(stdout_file.name)
    with tempfile.NamedTemporaryFile(prefix="telemaco-stderr-", delete=False) as stderr_file:
