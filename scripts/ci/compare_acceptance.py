#!/usr/bin/env python3
"""Compare acceptance-gate correctness and report latency changes.

Consumes two `acceptance/run.py --json` results, one for the base revision
and one for the candidate, and fails when a stage that passed on the base
no longer passes. Latency is reported, never enforced: CI runners are too
noisy for a timing threshold to mean anything on its own.
"""
...
def load_result(path: Path) -> dict:
    ...
    if not isinstance(value, dict) or not isinstance(value.get("results"), list):
        raise ValueError(f"{path} is not an acceptance result")
    return value
...
def result_map(value: dict) -> dict[str, dict]:
    ...
    for result in value["results"]:
        if not isinstance(result, dict) or not isinstance(result.get("name"), str):
            raise ValueError("acceptance result has an invalid stage")
        name = result["name"]
        if name in mapped:
            raise ValueError(f"duplicate acceptance stage: {name}")
        ...
        if (
            ...
            raise ValueError(f"invalid acceptance result for {name}")
        mapped[name] = result
    return mapped
...
def main() -> None:
    ...
    if len(base) != EXPECTED_STAGE_COUNT:
        raise SystemExit(
            f"base acceptance result has {len(base)} stages; expected {EXPECTED_STAGE_COUNT}"
        )
    if len(candidate) != EXPECTED_STAGE_COUNT:
        raise SystemExit(
            f"candidate acceptance result has {len(candidate)} stages; expected {EXPECTED_STAGE_COUNT}"
        )
    if set(base) != set(candidate):
        missing = sorted(set(base) - set(candidate))
        extra = sorted(set(candidate) - set(base))
        raise SystemExit(f"acceptance stage mismatch; missing={missing}, extra={extra}")
    ...
    if slow:
        ...
        for name, base_ms, candidate_ms, ratio in slow:
            ...
            print(f"::warning::acceptance latency {name}: {ratio:.2f}x base")
    add_summary("".join(lines))

    if regressions:
        raise SystemExit("candidate introduces new acceptance failures")
