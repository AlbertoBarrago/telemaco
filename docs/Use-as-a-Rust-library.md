The `telemaco` crate embeds the engine in a Rust program with a `Browser` / `Page` / `Element` API plus a cookie store, no CDP round-trips. It builds V8 from source, so it is a git dependency rather than a crates.io release.

## Add the dependency

```toml
[dependencies]
telemaco = { git = "https://github.com/AlbertoBarrago/telemaco" }
...
```toml
telemaco = { git = "https://github.com/AlbertoBarrago/telemaco", tag = "v0.1.7" }
```

## Quickstart

```rust
use telemaco::Browser;
...
```rust
use telemaco::{Browser, ResourceType};
use std::sync::Arc;
...
```rust
use telemaco::{Browser, InterceptResolution};
