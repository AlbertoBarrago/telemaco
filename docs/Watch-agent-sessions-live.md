...
Telemaco is headless, so an agent driving it through MCP, Puppeteer, or Playwright works invisibly. The CDP screencast surface lets you stream what the browser sees to a local tab while the agent works, which helps when supervising long tasks or debugging what an agent clicked.

This guide builds a small live viewer on top of `telemaco serve`. It uses only Node's built-in modules and its native WebSocket client (Node 21+).
...
<!doctype html>
<html>
<head><meta charset="utf-8"><title>Telemaco live</title>
...
With `telemaco serve` running and the viewer open:

1. Navigate from another client, for example `telemaco fetch https://example.com` through a separate worker, or any Puppeteer/Playwright/MCP session connected to port 9222.
