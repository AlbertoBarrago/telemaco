...
```bash
cargo nextest run --release --features render -p telemaco-cdp
cargo nextest run --release --features render -p telemaco-browser
```
...
`crates/telemaco-cdp/tests/cdp_*.rs` exercise CDP methods end-to-end with a real `dispatch` call and an in-process HTTP server.
...
`--verbose` on any subcommand is equivalent to `RUST_LOG=telemaco=info`.

## Driving the CDP server manually

```bash
telemaco serve --port 9222 --verbose
```
...
Lock contention in the dispatcher. Should not happen on current main. If it does, run with `RUST_LOG=telemaco_cdp=trace`, look for handlers that hold `v8_lock` across long awaits.

### `page.goto()` returns `null` from Puppeteer

Means `Network.requestWillBeSent` for the main document did not arrive with `requestId == loaderId`. Check `do_navigate` in `crates/telemaco-cdp/src/domains/page.rs`.
...
A handler is awaiting something that never resolves. Run with `RUST_LOG=telemaco=trace` and check the last log line before the hang.
...
perf record -F 99 -g -- ./target/release/telemaco fetch https://heavy-spa.example
...
```bash
heaptrack ./target/release/telemaco serve
```
