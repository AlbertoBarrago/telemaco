`telemaco fetch` loads a URL, runs its JavaScript, and prints the result.
...
```bash
telemaco fetch https://news.ycombinator.com \
...
```bash
telemaco fetch https://example.com --eval "(function(){
...
```bash
telemaco fetch https://my-spa.example --wait-until domcontentloaded --eval "document.title"
```
...
(When driving telemaco via Puppeteer or Playwright the default is `domcontentloaded` to match client expectations.)
