Telemaco speaks the Chrome DevTools Protocol over WebSocket. Puppeteer and
...
```
telemaco listening on ws://127.0.0.1:9222
```
...
Use `connectOverCDP`, not `connect`. Playwright's `connect` speaks Playwright's own protocol, which telemaco does not implement.
...
DOM-agent frameworks such as browser-use also connect: telemaco implements `DOMSnapshot.captureSnapshot` and `Target.targetInfoChanged` for perception, and `DOM.focus` so a focused field receives `Input.dispatchKeyEvent` keystrokes.
