...
Binary is at `./target/release/telemaco`.
...
```bash
OPENSSL_NO_VENDOR=1 cargo build --release -p telemaco-cli --bins --features render
```
...
```bash
./target/release/telemaco --version
./target/release/telemaco fetch https://example.com --eval "document.title"
```

Install system-wide:

```bash
cargo install --path crates/telemaco-cli --features render
```
