...
Create or edit a file under `crates/telemaco-cdp/src/domains/`:

```rust
// crates/telemaco-cdp/src/domains/my_domain.rs
...
In `crates/telemaco-cdp/src/dispatch.rs`, add a match arm:
...
`crates/telemaco-cdp/tests/cdp_my_domain.rs`:
...
```bash
cargo nextest run --release --features render -p telemaco-cdp my_domain
```
...
In `crates/telemaco-js/src/ops.rs`:
...
In `crates/telemaco-js/js/bootstrap.js`:
...
./target/release/telemaco fetch https://example.com --eval "
...
- CDP method with intercept: `crates/telemaco-cdp/src/domains/page.rs` `do_navigate`.
