`telemaco mcp` exposes telemaco as a Model Context Protocol server so MCP-capable clients (Claude Desktop, Claude Code, etc.) can drive it.
...
```bash
telemaco mcp --stealth --proxy http://proxy.example.com:8080
```
...
Restart Claude Desktop. The telemaco tools appear in the tool list.

## Claude Code

```bash
claude mcp add telemaco /path/to/telemaco mcp
```
