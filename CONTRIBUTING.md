# Contributing to Telemaco

Thanks for your interest in Telemaco. This guide covers how to build, test, and
...
Be respectful and constructive. We want Telemaco to be a welcoming project, so
...
Telemaco supports four release configurations. Keep all four building when you
change feature gates or shared code:

| Configuration | Command |
| --- | --- |
| Rendering | `cargo build --release -p telemaco-cli --bins --features render` |
| Rendering and stealth | `cargo build --release -p telemaco-cli --bins --features render,stealth` |
| No rendering | `cargo build --release -p telemaco-cli --bins --no-default-features` |
| No rendering, with stealth | `cargo build --release -p telemaco-cli --bins --no-default-features --features stealth` |
...
offline, and cannot collide with a server you already have running. CI runs it
for both your branch and its base, and blocks the pull request only when a
stage that passed on the base now fails.
...
2. Build the render binary, then run `render-repros/run.sh`. This captures each
   fixture in Telemaco and Chromium and runs the structural checks.
...
- The telemaco version or commit, plus OS and architecture.
...
Telemaco targets web scraping and AI-agent automation, and is heading toward a
