...
Embeddable Rust API for the [Telemaco](https://github.com/AlbertoBarrago/telemaco)
...
compiles Telemaco from source, including its embedded V8 (`deno_core`), so the
first build is large and slow.

```toml
[dependencies]
telemaco = { git = "https://github.com/AlbertoBarrago/telemaco", features = ["api"] }
