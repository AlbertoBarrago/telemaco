...
async fn a_rejected_child_frame_does_not_leave_js_references() {
    std::env::set_var("TELEMACO_ALLOW_PRIVATE_NETWORK", "1");
    std::env::set_var("TELEMACO_MAX_LIVE_FRAMES", "0");
    let base = spawn_server(STATIC_PARENT_HTML);
    ...
    for registry in [
        "__telemaco_frameObjects",
        "__telemaco_frameWindows",
        "__telemaco_frameElements",
    ] {
        assert_eq!(
...
async fn changing_iframe_src_releases_the_previous_realm() {
    ...
    for registry in ["__telemaco_frameWindows", "__telemaco_frameElements"] {
        assert_eq!(
            page.evaluate(&format!("Object.keys(globalThis.{registry}).length"))
                .as_f64(),
            Some(0.0),
