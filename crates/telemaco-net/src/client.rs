...
impl ResourceRequest {
    ...
    pub(crate) fn accept(&self) -> &'static str {
        match self.resource_type {
            ...
            // AVIF is intentionally omitted until telemaco's decoder can paint
...
pub(crate) fn response_too_large(url: &Url, limit: usize) -> TelemacoNetError {
    TelemacoNetError::ResponseTooLarge {
        url: url.to_string(),
        limit,
    }
}
...
pub fn env_allows_private_network() -> bool {
    matches!(
        std::env::var("TELEMACO_ALLOW_PRIVATE_NETWORK")
            .ok()
            .as_deref()
            .map(str::trim)
...
/// permitted (`--allow-private-network` or `TELEMACO_ALLOW_PRIVATE_NETWORK`) the
...
pub(crate) fn validate_url(url: &Url, allow_private_network: bool) -> Result<(), TelemacoNetError> {
    let allow_private_network = allow_private_network || env_allows_private_network();
    let scheme = url.scheme();
    if scheme != "http" && scheme != "https" && scheme != "file" {
        return Err(TelemacoNetError::Network(format!(
...
pub(crate) async fn fetch_file_url(
...
) -> Result<Response, TelemacoNetError> {
    let path = url
        .to_file_path()
        .map_err(|_| TelemacoNetError::Network("Invalid file URL".to_string()))?;
    ...
    let body = tokio::fs::read(&path)
        .await
        .map_err(|e| TelemacoNetError::Network(format!("Failed to read file: {}", e)))?;
...
fn response_header_value<'a>(
    ...
    name: &'static str,
    url: &Url,
) -> Result<Option<&'a str>, TelemacoNetError> {
    ...
    first.to_str().map(Some).map_err(|_| {
        TelemacoNetError::Cors(format!("{} returned an invalid {} header", url, name))
    })
}
...
async fn read_reqwest_body_limited(
    ...
    url: &Url,
    limit: usize,
) -> Result<Vec<u8>, TelemacoNetError> {
    ...
    while let Some(chunk) = response.chunk().await.map_err(|error| {
        TelemacoNetError::Network(format!("Failed to read body: {}", error))
    })? {
...
pub struct TelemacoHttpClient {
    ...
    /// through in addition to the `TELEMACO_ALLOW_PRIVATE_NETWORK` env var.
...
;
    ...
    /// (if any). Exposed so callers outside the `telemaco-net` crate — notably
    /// `op_fetch_url` in `telemaco-js` (#139) — can route their own reqwest
    /// requests through the same upstream proxy.
    pub fn proxy_url(&self) -> Option<&str> {
        self.proxy_url.as_deref()
    }

    pub async fn fetch(&self, url: &Url) -> Result<Response, TelemacoNetError> {
    ...
    pub async fn post_form(&self, url: &Url, body: &str) -> Result<Response, TelemacoNetError> {
    ...
    async fn fetch_with_profile_uncached(
        ...
        for _redirect_count in 0..max_redirects {
            ...
            if let Some(interceptor) = self.interceptor.read().await.as_ref() {
                match interceptor.intercept(&request_info).await {
                    ...
                    InterceptAction::Block => {
                        return Err(TelemacoNetError::Blocked(current_url.to_string()));
                    }
            ...
            let resp = req_builder.send().await.map_err(|e| {
                TelemacoNetError::Network(format!("{}: {}", current_url, e))
            })?;
            ...
            if status.is_redirection() {
                if let Some(location) = resp.headers().get(reqwest::header::LOCATION) {
                    let location_str = location.to_str().map_err(|_| {
                        TelemacoNetError::Network("Invalid redirect Location header".into())
                    })?;
                    let next_url = current_url.join(location_str).map_err(|e| {
                        TelemacoNetError::Network(format!("Invalid redirect URL: {}", e))
                    })?;
        ...
        Err(TelemacoNetError::TooManyRedirects(current_url.to_string()))
    }
...
impl Default for TelemacoHttpClient {
    fn default() -> Self {
        Self::new()
    }
}
...
mod ssrf_tests {
    use super::{
        ...
        CallbackRegistry, TelemacoHttpClient, TelemacoNetError, RequestCredentials, RequestMode,
    ...
    async fn cross_origin_font_sends_origin_and_omits_cross_origin_cookies() {
        ...
        let client = TelemacoHttpClient::with_full_options(jar.clone(), None, true);
    ...
    async fn credentialed_cors_rejects_wildcard_and_accepts_exact_origin() {
        ...
        let error = client
            ...
            .await
            .unwrap_err();
        assert!(matches!(error, TelemacoNetError::Cors(_)));
        client
    ...
    async fn same_origin_font_needs_no_cors_header_and_sends_cookies() {
        ...
        let client = TelemacoHttpClient::with_full_options(jar, None, true);
        client
            .fetch_resource_with_callbacks(
                &target,
                ResourceRequest::subresource(ResourceType::Font, &initiator),
    ...
    async fn accept_language_override_reaches_the_wire() {
        ...
        let client = TelemacoHttpClient::with_full_options(
    ...
    async fn response_limits_reject_content_length_and_streamed_overflow() {
        ...
        for _ in 0..2 {
            ...
            assert!(matches!(
                error,
                TelemacoNetError::ResponseTooLarge { limit: 6, .. }
            ));
    ...
    async fn cancellation_returns_active_requests_to_zero() {
        ...
        let client = Arc::new(TelemacoHttpClient::with_full_options(
            Arc::new(CookieJar::new()),
            None,
            true,
        ));
    ...
    async fn transport_timeout_returns_active_requests_to_zero() {
        let (target, started) = hanging_fixture().await;
        let mut client = TelemacoHttpClient::with_full_options(
            Arc::new(CookieJar::new()),
            None,
            true,
        );
    ...
    async fn no_store_vary_star_and_error_responses_are_not_reused() {
        for (status, headers) in [
            ...
            let initiator = url.join("/page.html").unwrap();
            let client =
                TelemacoHttpClient::with_full_options(Arc::new(CookieJar::new()), None, true);
            let request = ResourceRequest::subresource(ResourceType::Script, &initiator);
            client
