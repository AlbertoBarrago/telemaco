...
#[cfg(feature = "stealth")]
use crate::client::{
    CallbackRegistry, InFlightGuard, TelemacoNetError, RequestInfo, RequestMode,
...
impl StealthHttpClient {
    ...
    pub fn with_proxy(cookie_jar: Arc<CookieJar>, proxy_url: Option<&str>) -> Self {
        ...
        let mut builder = wreq::Client::builder()
            ...
            // resolver still honours TELEMACO_ALLOW_PRIVATE_NETWORK on its own.
    ...
    async fn fetch_with_profile(
        ...
        for _ in 0..20 {
            ...
            let resp = send_get_with_connection_reset_retry(req, &current_url)
                .await
                .map_err(|e| {
                    TelemacoNetError::Network(format!(
                        "{}: {} (source: {:?})",
                        current_url,
                        e,
                        e.source()
            ...
            if status.is_redirection() {
                if let Some(location) = resp.headers().get("location") {
                    let location_str = location.to_str().map_err(|_| {
                        TelemacoNetError::Network("Invalid redirect Location".into())
                    })?;
        ...
        Err(TelemacoNetError::TooManyRedirects(url.to_string()))
    }
    ...
    pub async fn send_single(
        ...
        let req_method = method
            .parse::<wreq::Method>()
            .map_err(|e| TelemacoNetError::Network(format!("invalid method '{}': {}", method, e)))?;
        ...
        let resp = req.send().await.map_err(|e| {
            TelemacoNetError::Network(format!("{}: {}", url, e))
        })?;
...
mod tests {
    ...
    use crate::client::{TelemacoNetError, SsrfGuardResolver};
    use crate::cookies::CookieJar;
    ...
    async fn stealth_post_does_not_retry_connection_reset() {
        ...
        assert!(matches!(error, TelemacoNetError::Network(_)));
