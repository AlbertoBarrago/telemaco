...
//! must not abort V8.
//!
//! Per maintainer comment on PR #36 (https://github.com/AlbertoBarrago/telemaco/pull/36#issuecomment-4328238087):
//!
...
//!
//! Run with `cargo test -p telemaco-cdp --test concurrent_navigations_with_fetch
//! -- --nocapture --ignored`.
    ...
    local
        .run_until(async {
            tokio::task::spawn_local(async move {
                let _ = telemaco_cdp::server::start(port).await;
            });
            tokio::task::spawn_local(async move {
