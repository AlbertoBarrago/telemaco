...
async fn every_session_that_added_the_binding_is_called() {
    ...
    cdp(
        ...
        3,
        "Runtime.evaluate",
        json!({"expression": "telemacoProbe('HELLO')"}),
        Some(&first),
    )
    .await;
    ...
    for (_, params) in &called {
        assert_eq!(params["name"], "telemacoProbe");
...
async fn a_removed_binding_is_not_called() {
    ...
    let gone = cdp(
        ...
        4,
        "Runtime.evaluate",
        json!({"expression": "typeof globalThis.telemacoProbe", "returnByValue": true}),
        Some(&session),
    )
    .await;
