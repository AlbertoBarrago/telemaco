...
async fn cdp_click_submit_button_is_vetoed_by_prevent_default_listener() {
    ...
    cdp(
        ...
        2,
        "Runtime.evaluate",
        json!({"expression": "globalThis.__telemaco_click_target = document.getElementById('b')"}),
        session_id,
    )
    .await;
...
// NotFoundError DOMException if it is not owned by the form. telemaco accepted
