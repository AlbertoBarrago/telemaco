...
//!    `X-Telemaco-Reason: max-connections`, not dropped with a bare reset;
...
//!
//! Run with `cargo nextest run -p telemaco-cdp -E 'test(max_connections_cap)'`.
...
fn max_connections_refuses_then_recovers() {
    ...
    local.block_on(&rt, async move {
        ...
        tokio::task::spawn_local(async move {
            let _ = telemaco_cdp::start_with_serve_options_and_limit(
                ws_port,
                "127.0.0.1",
                None,
                false,
        ...
        assert!(
            refused.contains("X-Telemaco-Reason: max-connections"),
