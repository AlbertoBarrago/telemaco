...
use telemaco_js::ops::InterceptedRequest;
use serde_json::json;
...
pub struct CdpContext {
    ...
    // navigation Telemaco clears execution contexts (via
    ...
    // Set of executionContextIds Telemaco has emitted via
...
pub async fn dispatch(req: &CdpRequest, ctx: &mut CdpContext) -> CdpResponse {
    ...
    // TELEMACO_CDP_COMMAND_TIMEOUT_MS tunes the bound (0 disables); the default
    // leaves headroom for the slowest legitimate navigation, which already
    // self-bounds via TELEMACO_NAV_TIMEOUT_MS plus the watchdog-bounded settle.
    let cmd_budget_ms: u64 = std::env::var("TELEMACO_CDP_COMMAND_TIMEOUT_MS")
        .ok()
    ...
    let cmd_watchdog = if cmd_budget_ms == 0 || is_v8_free_method(&req.method) {
        ...
        ctx.get_session_page(&req.session_id)
            ...
            .map(|h| {
                telemaco_js::cdp_watchdog::arm(h, std::time::Duration::from_millis(cmd_budget_ms))
            })
    };
    ...
    if let Some(wd) = cmd_watchdog {
        if telemaco_js::cdp_watchdog::disarm(wd) {
            tracing::warn!(
        ...
        for runtime_event in runtime_events {
            for session_id in sessions {
                let (method, params) = match &runtime_event {
                    telemaco_js::ops::RuntimeEvent::Console(event) => (
                    ...
                    telemaco_js::ops::RuntimeEvent::Exception(event) => (
