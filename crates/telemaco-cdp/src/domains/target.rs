    ...
    match method {
        ...
        "createTarget" => {
            ...
            if url_is_file_scheme(url) && !context.allow_file_access {
                return Err(
                    "Target.createTarget to file:// is disabled. Restart with `telemaco serve --allow-file-access` to enable.".to_string()
                );
            }
        ...
        // No multi-target lifecycle to manage: telemaco runs one page per session.
