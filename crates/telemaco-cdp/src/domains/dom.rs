...
fn resolve_node_id(page: &mut Page, params: &Value) -> Result<u64, String> {
    ...
    if let Some(oid) = params.get("objectId").and_then(|v| v.as_str()) {
        let code = format!(
            "(function() {{ var o = globalThis.__telemaco_objects && globalThis.__telemaco_objects[{}]; \
    ...
    match method {
        ...
        "describeNode" => {
                ...
                .or_else(|| params.get("backendNodeId").and_then(|v| v.as_u64()))
            {
                ...
                let code = format!(
                    "(function() {{ var o = globalThis.__telemaco_objects[{}]; if (!o) return -1; return (typeof o._nid === 'number') ? o._nid : -1; }})()",
                    crate::util::object_id_literal(oid)
                );
                let result = page.evaluate(&code);
        ...
        "resolveNode" => {
                ...
                .or_else(|| params.get("backendNodeId").and_then(|v| v.as_u64()))
            {
                ...
                let code = format!(
                    "(function() {{ var o = globalThis.__telemaco_objects[{}]; return (o && typeof o._nid === 'number') ? o._nid : -1; }})()",
                    crate::util::object_id_literal(oid)
                );
                let result = page.evaluate(&code);
                result.as_f64().map(|n| n as u64).unwrap_or(0)
            ...
            let js_code = format!(
                "(function() {{\
                    ...
                    if (globalThis._cache && globalThis._cache.has(nid)) {{\
                        ...
                        var t = +Deno.core.ops.op_dom('node_type', String(nid), '', globalThis.__telemaco_frameId >>> 0);\
        ...
        "removeNode" => Ok(json!({})),
        "focus" => {
            // No layout engine, but telemaco's JS focus() sets document.activeElement,
        ...
        "scrollIntoViewIfNeeded" => {
            let page = ctx.get_session_page_mut(session_id).ok_or("No page")?;
            let node_id = resolve_node_id(page, params)?;
            // Telemaco has no layout viewport to move, but the JS shim records
        ...
        "setFileInputFiles" => {
            ...
            if !page.context.allow_file_access {
                return Err("DOM.setFileInputFiles is disabled. Restart with `telemaco serve --allow-file-access` to enable local file uploads.".to_string());
            }
            let node_id = resolve_node_id(page, params)?;
            ...
            let code = format!(
                "(function() {{ var el = globalThis._wrap && globalThis._wrap({0}); \
                 if (el && globalThis.__telemaco_setInputFiles) {{ globalThis.__telemaco_setInputFiles(el, {1}); return true; }} return false; }})()",
                node_id, specs_json
            );
            let _ = page.evaluate(&code);
            Ok(json!({}))
...
mod tests {
    ...
    async fn scroll_into_view_if_needed_resolves_all_node_identifiers() {
        ...
        for params in [
            ...
            ctx.get_session_page_mut(&session)
                .unwrap()
                .evaluate("globalThis.__telemaco_click_target = null");
            ...
            let target_id = ctx
                .get_session_page_mut(&session)
                .unwrap()
                .evaluate("globalThis.__telemaco_click_target && globalThis.__telemaco_click_target.id");
    ...
    fn get_document_deep_tree_does_not_overflow() {
        use telemaco_dom::{DomTree, NodeData};
