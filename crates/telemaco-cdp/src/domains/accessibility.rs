...
fn compute_name(dom: &DomTree, node: &telemaco_dom::Node) -> Option<String> {
...
fn compute_value(dom: &DomTree, node: &telemaco_dom::Node) -> Option<String> {
...
fn contenteditable_keyword(node: &telemaco_dom::Node) -> Option<bool> {
...
fn is_content_editing_host(dom: &DomTree, node: &telemaco_dom::Node) -> bool {
...
fn compute_properties(_dom: &DomTree, node: &telemaco_dom::Node) -> Vec<Value> {
    if let NodeData::Element { name, attrs, .. } = &node.data {
        let tag = name.local.as_ref();
