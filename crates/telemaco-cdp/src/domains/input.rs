...
// so the whole snippet was dropped and nothing was inserted. telemaco-mcp
...
fn insert_text_js(text: &str) -> String {
    ...
    format!(
        "(function() {{\
            ...
            if (s == null) {{\
                globalThis.__telemaco_setFieldValue(t, 'value', v + ins);\
            }} else {{\
                s = Math.max(0, Math.min(s, v.length));\
                e = (e == null) ? s : Math.max(0, Math.min(e, v.length));\
                var lo = Math.min(s, e), hi = Math.max(s, e);\
                globalThis.__telemaco_setFieldValue(t, 'value', v.slice(0, lo) + ins + v.slice(hi));\
                var caret = lo + ins.length;\
                t.setSelectionRange(caret, caret);\
            }}\
            t.dispatchEvent(globalThis.__telemaco_markTrusted(new Event('input', {{bubbles:true}})));\
        }})()",
        text = literal,
    )
}
...
const BACKSPACE_JS: &str = "(function() {\
    ...
    var s = t.selectionStart, e = t.selectionEnd;\
    if (s == null) {\
        globalThis.__telemaco_setFieldValue(t, 'value', v.slice(0, -1));\
    } else {\
        s = Math.max(0, Math.min(s, v.length));\
        e = (e == null) ? s : Math.max(0, Math.min(e, v.length));\
        if (s !== e) {\
            var lo = Math.min(s, e), hi = Math.max(s, e);\
            globalThis.__telemaco_setFieldValue(t, 'value', v.slice(0, lo) + v.slice(hi));\
            t.setSelectionRange(lo, lo);\
        } else if (s > 0) {\
            globalThis.__telemaco_setFieldValue(t, 'value', v.slice(0, s - 1) + v.slice(s));\
        ...
        "dispatchMouseEvent" => {
            ...
            if event_type == "mousePressed" {
                if let Some(page) = ctx.get_session_page_mut(session_id) {
                    let code = format!(
                        "(function() {{\
                            var target = (document.elementFromPoint && document.elementFromPoint({x},{y})) || globalThis.__telemaco_click_target || document.activeElement || document.body;\
                            if (!target) return;\
                            globalThis.__telemaco_click_target = target;\
                ...
                let moved_frame = if let Some(page) = ctx.get_session_page_mut(session_id) {
                    let code = format!(
                        "(function() {{\
                            var target = (document.elementFromPoint && document.elementFromPoint({x},{y})) || globalThis.__telemaco_click_target || document.activeElement || document.body;\
                            if (!target) return;\
                            var down = globalThis.__telemaco_mouse_down;\
                            globalThis.__telemaco_mouse_down = null;\
                            var evt = globalThis.__telemaco_markTrusted(new MouseEvent('mouseup', {{bubbles:true,cancelable:true,view:globalThis,clientX:{x},clientY:{y},button:{button_code},buttons:0,detail:{click_count},altKey:{alt_key},ctrlKey:{ctrl_key},metaKey:{meta_key},shiftKey:{shift_key}}}));\
                            target.dispatchEvent(evt);\
                            ...
                            if (globalThis.__telemaco_isDisabled(clickTarget)) return;\
                            ...
                            var click = globalThis.__telemaco_markTrusted(new MouseEvent('click', {{bubbles:true,cancelable:true,view:globalThis,clientX:{x},clientY:{y},button:0,buttons:0,detail:{click_count},altKey:{alt_key},ctrlKey:{ctrl_key},metaKey:{meta_key},shiftKey:{shift_key}}}));\
                            ...
                            if (checkable && clickTarget.checked !== oldChecked) {{\
                                try {{ clickTarget.dispatchEvent(globalThis.__telemaco_markTrusted(new Event('input', {{bubbles:true}}))); }} catch(e) {{}}\
                                try {{ clickTarget.dispatchEvent(globalThis.__telemaco_markTrusted(new Event('change', {{bubbles:true}}))); }} catch(e) {{}}\
                                return;\
                            }}\
                ...
                if let Some(page) = ctx.get_session_page_mut(session_id) {
                    let code = format!(
                        "(function() {{\
                            ...
                            if (!target) return;\
                            var wheel = globalThis.__telemaco_markTrusted(new WheelEvent('wheel', {{bubbles:true,cancelable:true,view:globalThis,clientX:{x},clientY:{y},deltaX:{delta_x},deltaY:{delta_y},deltaMode:0,altKey:{alt_key},ctrlKey:{ctrl_key},metaKey:{meta_key},shiftKey:{shift_key}}}));\
                ...
                match event_type {
                    "keyDown" | "rawKeyDown" => {
                        ...
                        if key == "Enter" {
                            ...
                            let js = "(function() {\
                                ...
                                if (!target) return;\
                                target.dispatchEvent(globalThis.__telemaco_markTrusted(new KeyboardEvent('keypress', {bubbles:true,key:'Enter',code:'Enter'})));\
                                if (target.localName === 'textarea') {\
                                    globalThis.__telemaco_setFieldValue(target, 'value', (target.value || '') + '\\n');\
                                    target.dispatchEvent(globalThis.__telemaco_markTrusted(new Event('input', {bubbles:true})));\
                                } else {\
                    ...
                    "keyUp" => {
                        let js = format!(
                            "(function() {{\
                                var target = document.activeElement || document.body;\
                                var evt = globalThis.__telemaco_markTrusted(new KeyboardEvent('keyup', {{bubbles:true,key:'{key}',code:'{code}'}}));\
                                target.dispatchEvent(evt);\
                            }})()",
                            key = key.replace('\\', "\\\\").replace('\'', "\\'"),
                            code = code.replace('\\', "\\\\").replace('\'', "\\'"),
