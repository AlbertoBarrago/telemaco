...
struct ParsedPdfOptions {
    raster: telemaco_browser::RasterPdfOptions,
...
#[cfg(feature = "render")]
fn parse_page_ranges(value: &str) -> Result<Vec<telemaco_browser::RasterPdfPageRange>, String> {
    ...
    for (index, part) in value.split(',').enumerate() {
        ...
        ranges.push(telemaco_browser::RasterPdfPageRange { start, end });
    }
    ...
    let mut normalized: Vec<telemaco_browser::RasterPdfPageRange> = Vec::new();
    for range in ranges {
        ...
        normalized.push(telemaco_browser::RasterPdfPageRange {
            start: Some(start),
            end: range.end,
        });
    }
...
fn parse_options(params: &Value) -> Result<ParsedPdfOptions, String> {
    ...
    let defaults = telemaco_browser::RasterPdfOptions::default();
    Ok(ParsedPdfOptions {
        raster: telemaco_browser::RasterPdfOptions {
...
pub async fn print_to_pdf(
    ...
    #[cfg(feature = "render")]
    {
        ...
        let mut response = json!({
            "telemacoPrintMode": "print-media-raster",
            "telemacoPrintBackground": options.requested_print_background,
            "telemacoRequestedPrintBackground": options.requested_print_background,
            "telemacoTaggedPDF": false,
            "telemacoRequestedTaggedPDF": options.requested_tagged_pdf,
            "telemacoCapabilities": {
                "cssPagedMedia": false,
        ...
        response["telemacoIgnoredOptions"] = json!(ignored_options);
...
mod tests {
    ...
    fn parser_accepts_standard_client_defaults_and_rejects_unrepresented_features() {
        ...
        assert_eq!(
            ...
            vec![
                telemaco_browser::RasterPdfPageRange {
                    start: Some(1),
                    end: Some(1),
                },
                telemaco_browser::RasterPdfPageRange {
    ...
    async fn print_to_pdf_returns_paginated_pdf_with_requested_media_box() {
        ...
        .await
        .expect("raster PDF");
        assert_eq!(response["telemacoPrintMode"], "print-media-raster");
        assert_eq!(response["telemacoPrintBackground"], true);
        use base64::Engine as _;
        ...
        assert_eq!(streamed["telemacoPrintBackground"], false);
        assert_eq!(streamed["telemacoRequestedPrintBackground"], false);
        assert_eq!(
            streamed["telemacoCapabilities"]["honorsPrintBackground"],
            true
        );
        assert_eq!(streamed["telemacoCapabilities"]["honorsScale"], true);
        assert_eq!(streamed["telemacoCapabilities"]["honorsPrintMedia"], true);
        assert_eq!(streamed["telemacoCapabilities"]["pageRanges"], true);
        assert_eq!(streamed["telemacoTaggedPDF"], false);
        assert_eq!(streamed["telemacoRequestedTaggedPDF"], true);
        assert_eq!(
            streamed["telemacoIgnoredOptions"],
