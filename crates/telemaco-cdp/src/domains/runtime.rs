...
use telemaco_js::runtime::RemoteObjectInfo;
...
pub async fn handle(
    ...
    match method {
        ...
        "getProperties" => {
            ...
            //      __telemaco_objects so later callFunctionOn can resolve it.
            ...
            if let Some(oid) = object_id {
                ...
                let code = format!(
                    "(function() {{\
                        var obj = globalThis.__telemaco_objects[{oid}];\
                        ...
                        return keys.map(function(k) {{\
                            ...
                            var childOid = {oid} + '::' + k;\
                            globalThis.__telemaco_objects[childOid] = v;\
        ...
        "addBinding" => {
            ...
            if is_valid_binding_name(name) {
                ...
                let key = format!("__telemaco_binding__{}", name);
...
/// context Telemaco has not advertised. Returns `Ok(())` when the parameter is
...
mod tests {
    ...
    /// target exists. Real Chrome replies with `{}`; before the fix Telemaco
