use telemaco_browser::lifecycle::WaitUntil;
...
fn child_frame_value(page_frame_id: &str, frame: &telemaco_js::frame::FrameRealm) -> Value {
...
pub fn child_frame_values(page: &telemaco_browser::Page) -> Vec<Value> {
    fn walk(page: &telemaco_browser::Page, parent_frame_id: u32, out: &mut Vec<Value>) {
        for frame in page
            .frames
            .iter()
...
fn frame_tree(page: &telemaco_browser::Page, loader_id: &str) -> Value {
    fn children(page: &telemaco_browser::Page, parent_frame_id: u32) -> Vec<Value> {
        page.frames
            .iter()
            .filter(|frame| frame.parent_frame_id() == parent_frame_id)
            .map(|frame| {
...
#[cfg(feature = "render")]
fn capture_error_message(error: telemaco_browser::CaptureError) -> String {
    match error {
        telemaco_browser::CaptureError::InvalidRegion => {
            "Page.captureScreenshot received an invalid capture region".to_string()
        }
        telemaco_browser::CaptureError::AllocationLimitExceeded => {
            "Page.captureScreenshot bitmap is too large".to_string()
        }
        telemaco_browser::CaptureError::PaintFailed => {
            "Page.captureScreenshot failed: the page has no retained DOM surface to render"
                .to_string()
        }
        telemaco_browser::CaptureError::EncodeFailed => {
...
fn chromium_clip_region(
...
) -> Result<telemaco_browser::CaptureRegion, String> {
    ...
    Ok(telemaco_browser::CaptureRegion::with_output_size(
        clip.x as f32,
        clip.y as f32,
        width as f32,
        height as f32,
...
fn encode_long_full_page_png(
    ...
    if native_width == 0
        || native_height == 0
        || native_width > u64::from(telemaco_js::MAX_CAPTURE_DIMENSION)
        || !output_width_value.is_finite()
        || !output_height_value.is_finite()
        || output_width_value <= 0.0
        || output_height_value <= 0.0
        || output_width_value > f64::from(telemaco_js::MAX_CAPTURE_DIMENSION)
    ...
    let max_native_rows = (telemaco_js::MAX_CAPTURE_PIXELS / native_width)
        .min(u64::from(telemaco_js::MAX_CAPTURE_DIMENSION));
    let max_output_rows = (telemaco_js::MAX_CAPTURE_PIXELS / u64::from(output_width))
        .min(u64::from(telemaco_js::MAX_CAPTURE_DIMENSION));
        ...
        .map_err(|error| format!("Page.captureScreenshot striped PNG header failed: {error}"))?;
    {
        ...
        while output_y < output_height {
            ...
            let region = telemaco_browser::CaptureRegion::with_output_size(
                0.0,
                css_y as f32,
                width,
                css_height as f32,
...
pub(crate) async fn prepare_capture_resources_if_requested(
    page: &mut telemaco_browser::Page,
) {
    let Some(deadline_ms) = std::env::var("TELEMACO_RENDER_RESOURCE_DEADLINE_MS")
        .ok()
...
/// intentionally command-driven until Telemaco has a compositor frame pump.
#[cfg(feature = "render")]
pub(crate) fn queue_screencast_frame(
    ...
    let (viewport, scroll, png, activity_generation) = {
        ...
        let scroll = page
            ...
            .unwrap_or((0.0, 0.0));
        telemaco_browser::validate_capture_region(telemaco_browser::CaptureRegion::new(
            scroll.0 as f32,
            scroll.1 as f32,
            viewport.0,
            viewport.1,
...
/// is Telemaco's single-threaded equivalent until it owns a real compositor.
///
...
pub fn emit_navigation_events(
    ...
    page_url: &str,
    page_id: &str,
    network_events: &[telemaco_browser::NetworkEvent],
...
async fn do_navigate(
    ...
    // any file the telemaco process can read. Opt in via
    // `telemaco serve --allow-file-access` when local-HTML
    ...
    if url_is_file_scheme(url) && !allow_file_access {
        return Err(
            "Page.navigate to file:// is disabled. Restart with `telemaco serve --allow-file-access` to enable.".to_string()
        );
    }
...
pub async fn handle(
    ...
    match method {
        ...
        // Telemaco does not download files to disk, so there is no behavior to
        ...
        "startScreencast" => {
            #[cfg(feature = "render")]
            {
                ...
                Ok(json!({
                    "telemacoFrameSource": "activity-driven",
                    "telemacoAutonomousFrames": true,
                }))
            }
        ...
        "captureScreenshot" => {
            #[cfg(feature = "render")]
            {
                ...
                if !options.from_surface {
                    return Err(
                        "Page.captureScreenshot fromSurface=false is not supported: Telemaco has no separate browser-window compositor surface"
                            .to_string(),
                    );
                }
                ...
                let region = if let Some(clip) = options.clip {
                    ...
                    Some(telemaco_browser::CaptureRegion::new(
                        0.0,
                        0.0,
                        content_size.0,
                        content_size.1,
                ...
                let (png, png_is_final_encoding) = match region {
                    ...
                    {
                        Ok(png) => (png, false),
                        Err(telemaco_browser::CaptureError::AllocationLimitExceeded)
                    ...
                    None => {
                        telemaco_browser::validate_capture_region(
                            telemaco_browser::CaptureRegion::new(
                                scroll.0 as f32,
                                scroll.1 as f32,
                                viewport.0,
                                viewport.1,
        ...
        "captureSnapshot" => {
            ...
            Err(format!(
                "Page.{method} is not supported by Telemaco: no layout or paint engine. \
                 For visual snapshots, drive a real headless Chromium for the \
                 screenshot leg of your pipeline and use Telemaco for the scraping leg."
            ))
        }
...
mod tests {
    ...
    fn runtime_network_events_reuse_the_document_loader_without_lifecycle_replay() {
        ...
        let event = telemaco_browser::NetworkEvent {
    ...
    async fn screencast_initial_frame_metadata_and_encoding_match_options() {
        ...
        .await
        .expect("start screencast");
        assert_eq!(result["telemacoFrameSource"], "activity-driven");
        assert_eq!(result["telemacoAutonomousFrames"], true);
