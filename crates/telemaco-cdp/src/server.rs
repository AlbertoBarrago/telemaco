...
const CONNECTION_LIMIT_RESPONSE: &str = "HTTP/1.1 503 Service Unavailable\r\n\
    Content-Length: 0\r\nConnection: close\r\n\
    X-Telemaco-Reason: max-connections\r\n\r\n";
...
pub async fn start_with_serve_options_and_limit(
    ...
    info!("Telemaco CDP server listening on ws://{}:{}", host, port);
    info!(
    ...
    let mut bctx = telemaco_browser::BrowserContext::with_storage_and_network(
        "default".to_string(),
        proxy,
        stealth,
        user_agent,
    ...
    drop(telemaco_js::runtime::TelemacoJsRuntime::new());
...
fn run_connection(
    ...
    context_template: Arc<telemaco_browser::BrowserContext>,
    persistence_context: Arc<telemaco_browser::BrowserContext>,
...
fn cookie_key(cookie: &telemaco_net::CookieInfo) -> (String, String, String) {
    (
        cookie.domain.clone(),
        cookie.name.clone(),
        cookie.path.clone(),
...
fn cookie_values_match(
    left: &telemaco_net::CookieInfo,
    right: &telemaco_net::CookieInfo,
) -> bool {
...
fn merge_cookie_delta(
    destination: &telemaco_net::CookieJar,
    initial: &[telemaco_net::CookieInfo],
    current: &[telemaco_net::CookieInfo],
) {
...
async fn cdp_processor(
    ...
    default_context: Arc<telemaco_browser::BrowserContext>,
    shutdown_notify: Arc<Notify>,
) {
    let mut ctx = CdpContext::new_with_shared_context(default_context);
    let (itx, irx) = mpsc::unbounded_channel::<telemaco_js::ops::InterceptedRequest>();
    ctx.intercept_tx = Some(itx);
    let mut intercept_rx: Option<mpsc::UnboundedReceiver<telemaco_js::ops::InterceptedRequest>> = Some(irx);
    let mut intercepted_paused: HashMap<String, tokio::sync::oneshot::Sender<telemaco_js::ops::InterceptResolution>> = HashMap::new();
    ...
    // Telemaco has no separate compositor thread yet, so active screencasts get
                ...
                Some(intercepted) = async {
                        ...
                        (live_page_route.as_ref(), connection_reply_tx.as_ref())
                    {
                        ...
                        let _ = intercepted.resolver.send(
                            telemaco_js::ops::InterceptResolution::Fail {
                                reason: "Aborted".into(),
                            },
                        );
                    }
...
fn emit_intercepted_request(
    intercepted: telemaco_js::ops::InterceptedRequest,
    frame_id: &str,
    session_id: Option<String>,
    reply_tx: &mpsc::UnboundedSender<String>,
    intercepted_paused: &mut HashMap<
        String,
        tokio::sync::oneshot::Sender<telemaco_js::ops::InterceptResolution>,
    >,
) {
    tracing::info!(
...
fn handle_fetch_resolution(
    ...
    _ctx: &mut CdpContext,
    reply_tx: &mpsc::UnboundedSender<String>,
    intercepted_paused: &mut HashMap<String, tokio::sync::oneshot::Sender<telemaco_js::ops::InterceptResolution>>,
) -> bool {
    if let Ok(req) = serde_json::from_str::<CdpRequest>(text) {
        ...
        if let Some(resolver) = intercepted_paused.remove(request_id) {
            ...
            let resolution = match method {
                "Fetch.continueRequest" => telemaco_js::ops::InterceptResolution::Continue {
                ...
                "Fetch.fulfillRequest" => {
                    ...
                    let headers = req.params.get("responseHeaders")
                        ...
                        .unwrap_or_default();
                    telemaco_js::ops::InterceptResolution::Fulfill { status, headers, body }
                }
                "Fetch.failRequest" => {
                    let reason = req.params.get("errorReason").and_then(|v| v.as_str()).unwrap_or("Failed").to_string();
                    telemaco_js::ops::InterceptResolution::Fail { reason }
                }
                _ => return false,
            };
...
async fn process_with_interception(
    ...
    intercept_rx: &mut Option<mpsc::UnboundedReceiver<telemaco_js::ops::InterceptedRequest>>,
    intercepted_paused: &mut HashMap<String, tokio::sync::oneshot::Sender<telemaco_js::ops::InterceptResolution>>,
    ...
    let (nav_done_tx, mut nav_done_rx) = mpsc::channel::<(telemaco_browser::Page, Result<(), String>)>(1);
    ...
    let page_back: Option<telemaco_browser::Page>;
...
mod tests {
    ...
    use telemaco_net::{CookieInfo, CookieJar};
    use serde_json::json;
    use std::collections::HashMap;
    ...
    async fn page_runtime_advances_while_cdp_client_is_silent() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                // and Telemaco's connection-owned page pump must do the same.
    ...
    fn fetch_resolution_is_handled_once_by_the_outer_processor() {
        ...
        assert!(matches!(
            ...
            Ok(telemaco_js::ops::InterceptResolution::Continue { .. })
        ));
