...
#[command(
    name = "telemaco",
    version = env!("TELEMACO_BUILD_VERSION"),
    about = "Telemaco - A lightweight headless browser for web scraping and automation",
)]
struct Args {
    ...
    /// Equivalent to `TELEMACO_ALLOW_PRIVATE_NETWORK=1` but per-process
...
enum Command {
    Serve {
        ...
        #[arg(long, default_value_t = telemaco_cdp::DEFAULT_MAX_CONNECTIONS)]
...
fn print_banner(port: u16) {
    println!(
        r#"
 _                                         
| |_   ___   _     __ _ ___       __ _    ___   ___ 
| __| / _ \ | |   / _` / __|     / _` |  / __| / _ \
| |_ |  __/ | |  | (_| \__ \    | (_| | | (__ | (_) |
 \__| \___| |_|   \__,_|___/     \__,_|  \___| \___/

  telemaco - headless browser v{}
  CDP server: ws://127.0.0.1:{}/devtools/browser
"#,
        env!("TELEMACO_BUILD_VERSION"),
        port
    );
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Europe/Berlin; set TELEMACO_TIMEZONE to match the exit IP's region. An existing
    ...
    if let Some(tz) = std::env::var("TELEMACO_TIMEZONE")
        .ok()
    ...
    telemaco_js::set_v8_flags(&v8_flags);

    // The js-side fetch path (op_fetch_url) reads TELEMACO_ALLOW_PRIVATE_NETWORK
    ...
    match args.command {
        Some(Command::Serve {
            ...
            // Fall back to TELEMACO_PROXY so a proxy can be supplied without
            ...
            if workers > 1 {
                ...
                telemaco_cdp::start_with_serve_options_and_limit(
                    port,
                    &host,
                    proxy,
                    stealth,
...
async fn run_multi_worker_serve(
    ...
    for i in 0..workers {
        ...
        if let Some(ref p) = proxy {
            ...
            // to any local user; TELEMACO_PROXY is only readable by the owner
            // (issue #366). The worker's serve path reads this env as a fallback.
            cmd.env("TELEMACO_PROXY", p);
        }
        if let Some(ref ua) = user_agent {
...
async fn run_fetch(
    ...
    let screenshot_viewport = screenshot.as_ref().map(|_| {
        let width = std::env::var("TELEMACO_SHOT_W")
            .ok()
            .and_then(|v| v.parse::<f32>().ok())
            .unwrap_or(1280.0);
        let height = std::env::var("TELEMACO_SHOT_H")
            .ok()
            .and_then(|v| v.parse::<f32>().ok())
            .unwrap_or(720.0);
        (width, height)
    ...
    let wait_condition = telemaco_browser::lifecycle::WaitUntil::from_str(wait_until);
    ...
    let eval_at_capture_boundary = screenshot.is_some()
        && eval.is_some()
        && std::env::var("TELEMACO_SHOT_EVAL_AT_CAPTURE").is_ok_and(|value| value == "1");
    let controlled_scroll_request = screenshot.as_ref().and_then(|_| {
        let raw_y = std::env::var("TELEMACO_SHOT_SCROLL_Y").ok()?;
        let x = match std::env::var("TELEMACO_SHOT_SCROLL_X") {
    ...
    // normal fetch returns first and the process exits before this fires.
    {
        ...
        std::thread::spawn(move || {
            ...
            eprintln!(
                "telemaco: hard timeout exceeded ({}s); forcing exit",
                hard.as_secs()
            );
    ...
    if let Some(ref path) = screenshot {
        #[cfg(feature = "render")]
        {
            let resource_deadline_ms = std::env::var("TELEMACO_RENDER_RESOURCE_DEADLINE_MS")
                .ok()
            ...
            // TELEMACO_SHOT_W / TELEMACO_SHOT_H override it (e.g. a tall viewport to
            ...
            let requested_animation_sample = std::env::var("TELEMACO_SHOT_ANIMATION_TIME_MS")
                .ok()
                .map(|raw| {
                    let milliseconds = raw.parse::<f32>().map_err(|_| {
                        anyhow::anyhow!(
                            "TELEMACO_SHOT_ANIMATION_TIME_MS must be a finite non-negative number"
                        )
                    })?;
                    ...
                    Ok(telemaco_browser::AnimationSampleTime { milliseconds })
                })
                .transpose()?;
            ...
            // before observing image/font readiness. Telemaco resolves retained
            // render resources during prepare/paint, so sampling first would
            // compare pre-paint Telemaco state with post-load Chromium state.
            // Keep this private opt-in out of ordinary CLI screenshots.
            let warmup_capture =
                std::env::var("TELEMACO_SHOT_RESOURCE_WARMUP").is_ok_and(|value| value == "1");
            if warmup_capture {
...
async fn fetch_original_response(
    ...
    timeout_secs: u64,
    stealth: bool,
) -> anyhow::Result<telemaco_net::Response> {
    ...
    // here the same way TelemacoHttpClient::fetch_with_method excludes it
    // internally.
    if stealth && url.scheme() != "file" {
        #[cfg(feature = "stealth")]
        {
            let client = telemaco_net::StealthHttpClient::with_proxy(
                Arc::new(telemaco_net::CookieJar::new()),
                proxy.as_deref(),
            );
    ...
    let client = telemaco_net::TelemacoHttpClient::with_options(
        Arc::new(telemaco_net::CookieJar::new()),
        proxy.as_deref(),
    );
...
fn extract_readable_text(dom: &telemaco_dom::DomTree, node_id: telemaco_dom::NodeId) -> String {
    use telemaco_dom::NodeData;
    ...
    // of the serialize/textContent paths made iterative in telemaco-dom). A
    ...
    for (i, url) in urls.into_iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            ...
            let mut child = match TokioCommand::new(worker_path.as_ref())
                ...
                .env("TELEMACO_PROXY", proxy.as_deref().unwrap_or(""))
                .env("TELEMACO_STEALTH", if stealth { "1" } else { "" })
                .env("TELEMACO_OBEY_ROBOTS", if obey_robots { "1" } else { "" })
                .spawn()
            {
                Ok(c) => c,
                Err(e) => {
...
fn extract_assets(dom: &telemaco_dom::DomTree, base_url: Option<&url::Url>) -> String {
...
mod tests {
    ...
    fn concurrency_rejects_zero() {
        ...
        let err =
            Args::try_parse_from(["telemaco", "fetch", "--file", "u.txt", "--concurrency", "0"]);
        assert!(err.is_err());
    }
    ...
    fn read_urls_skips_blanks_and_comments() {
        ...
        let path = dir.join(format!("telemaco_urls_{}.txt", std::process::id()));
        std::fs::write(
            &path,
    ...
    async fn fetch_original_bytes_returns_file_contents_verbatim() {
        ...
        let path = std::env::temp_dir().join(format!(
            "telemaco-fetch-original-test-{}.png",
            std::process::id()
        ));
    ...
    async fn fetch_original_bytes_file_url_ignores_stealth_flag() {
        ...
        let path = std::env::temp_dir().join(format!(
            "telemaco-fetch-original-stealth-test-{}.png",
            std::process::id()
        ));
        let _ = tokio::fs::remove_file(&path).await;
        tokio::fs::write(&path, PNG_BYTES)
    ...
    async fn write_or_print_writes_output_file_with_tokio_fs() {
        let path = std::env::temp_dir().join(format!(
            "telemaco-fetch-output-test-{}.txt",
            std::process::id()
        ));
        let _ = tokio::fs::remove_file(&path).await;
    ...
    fn parsed_fetch_with_quiet_flag_is_detected() {
        let args = Args::try_parse_from(["telemaco", "fetch", "--quiet", "https://example.com"])
    ...
    fn parsed_fetch_without_quiet_is_not_detected() {
        let args = Args::try_parse_from(["telemaco", "fetch", "https://example.com"])
    ...
    fn parsed_serve_command_is_not_quiet() {
        let args = Args::try_parse_from(["telemaco", "serve"]).expect("clap should accept serve");
        assert!(!is_quiet_command(&args.command));
    }
    ...
    fn parsed_v8_flags_empty_string_is_accepted() {
        let args =
            Args::try_parse_from(["telemaco", "--v8-flags", "", "fetch", "https://example.com"])
    ...
    fn parsed_fetch_quiet_resolves_to_off_filter() {
        let args =
            Args::try_parse_from(["telemaco", "fetch", "--quiet", "https://example.com"]).unwrap();
    ...
    fn fetch_wait_distinguishes_adaptive_default_from_fixed_delay() {
        let default = Args::try_parse_from(["telemaco", "fetch", "https://example.com"]).unwrap();
        ...
        let fixed =
            Args::try_parse_from(["telemaco", "fetch", "https://example.com", "--wait", "0"])
                .unwrap();
        match fixed.command {
    ...
    fn configured_fetch_timeout(args: Args) -> std::time::Duration {
        ...
        let context = std::sync::Arc::new(
            telemaco_browser::BrowserContext::with_storage_and_network(
        ...
        let mut page = telemaco_browser::Page::new("cli-timeout-test".to_string(), context);
    ...
    fn fetch_default_navigation_budget_remains_thirty_seconds() {
        let args = Args::try_parse_from(["telemaco", "fetch", "https://example.com"]).unwrap();
        assert_eq!(
            configured_fetch_timeout(args),
