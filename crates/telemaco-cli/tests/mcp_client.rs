...
const TELEMACO: &str = env!("CARGO_BIN_EXE_telemaco");
...
/// rather than Telemaco navigation, snapshots, or evaluation.
...
impl McpClient {
    fn spawn() -> Self {
        let mut child = Command::new(TELEMACO)
            .args(["mcp"])
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("failed to spawn telemaco mcp");
...
fn test_initialize() {
    ...
    assert_eq!(resp["result"]["serverInfo"]["name"], "telemaco-mcp");
}
