...
fn output_directory() -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "telemaco-screenshot-after-eval-{}-{unique}",
        std::process::id()
    ))
}
...
fn screenshot_is_captured_after_eval_scrolls_the_live_page() {
    ...
    let run = |path: &std::path::Path, eval: Option<&str>| {
        let mut command = Command::new(env!("CARGO_BIN_EXE_telemaco"));
        command
            .args(["fetch", url, "--screenshot"])
            .arg(path)
            .args(["--wait", "0", "--timeout", "5", "--quiet"])
            .env("TELEMACO_SHOT_W", "100")
            .env("TELEMACO_SHOT_H", "80");
        if let Some(expression) = eval {
            command.args(["--eval", expression]);
        }
        command.output().expect("run telemaco fetch")
    };
...
fn screenshots_use_live_animation_time_unless_harness_pins_a_sample() {
    ...
    let run = |path: &std::path::Path, animation_time: Option<&str>| {
        ...
        command
            ...
            .env("TELEMACO_SHOT_W", "80")
            .env("TELEMACO_SHOT_H", "60")
            .env_remove("TELEMACO_SHOT_ANIMATION_TIME_MS");
        if let Some(milliseconds) = animation_time {
            command.env("TELEMACO_SHOT_ANIMATION_TIME_MS", milliseconds);
        }
        command.output().expect("run telemaco fetch")
    };
...
fn paired_capture_reasserts_scroll_after_the_post_eval_settle() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_telemaco"))
        .args(["fetch", url, "--screenshot"])
        .arg(&screenshot)
        .args([
            "--eval",
        ...
        .env("TELEMACO_SHOT_W", "100")
        .env("TELEMACO_SHOT_H", "80")
        .env("TELEMACO_SHOT_SCROLL_X", "0")
        .env("TELEMACO_SHOT_SCROLL_Y", "80")
        .output()
        .expect("run telemaco fetch");
    assert!(
...
fn paired_capture_evaluates_state_after_final_scroll_reassert() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_telemaco"))
        ...
        .env("TELEMACO_SHOT_SCROLL_X", "0")
        .env("TELEMACO_SHOT_SCROLL_Y", "80")
        .env("TELEMACO_SHOT_EVAL_AT_CAPTURE", "1")
        .env("TELEMACO_SHOT_RESOURCE_WARMUP", "1")
        .output()
        .expect("run telemaco fetch");
    assert!(
        output.status.success(),
