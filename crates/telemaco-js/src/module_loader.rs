...
pub struct TelemacoModuleLoader {
    pub base_url: String,
    ...
    page_state: Option<Weak<RefCell<TelemacoState>>>,
    /// Directly-constructed loaders still use Telemaco's network policy and
    /// connection pool; they simply have an isolated cookie jar.
    standalone_client: Option<Arc<telemaco_net::TelemacoHttpClient>>,
...
impl TelemacoModuleLoader {
    ...
    fn with_proxy_and_import_map(
        ...
        let standalone_client = Arc::new(telemaco_net::TelemacoHttpClient::with_options(
            Arc::new(telemaco_net::CookieJar::new()),
            proxy_url.as_deref(),
        ));
        TelemacoModuleLoader {
    ...
    pub(crate) fn with_page_state(
        base_url: &str,
        proxy_url: Option<String>,
        page_state: &Rc<RefCell<TelemacoState>>,
        import_map: Rc<RefCell<ImportMap>>,
    ) -> Self {
        TelemacoModuleLoader {
            base_url: base_url.to_string(),
...
impl ModuleLoader for TelemacoModuleLoader {
    fn resolve(
        &self,
        specifier: &str,
        referrer: &str,
        ...
        ModuleLoadResponse::Async(Pin::from(Box::new(async move {
            ...
            match page_network {
                Ok((client, stealth, callbacks)) => {
                    ...
                    let request =
                        telemaco_net::ResourceRequest::module_script(&document_url, &referrer);
                    #[cfg(feature = "stealth")]
