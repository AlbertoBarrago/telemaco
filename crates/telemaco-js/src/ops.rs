...
/// Configurable via `TELEMACO_FETCH_MAX_BODY_BYTES`.
...
// Fallback cache for runtimes that have no owning TelemacoHttpClient, such as
...
/// Shared HTTP client cache for any code in telemaco-js that needs a
...
fn build_request_client(proxy_url: Option<&str>) -> Result<reqwest::Client, String> {
    ...
    // workspace; TELEMACO_FETCH_TIMEOUT_MS overrides it for tighter cloud limits.
...
/// transport. Browser pages always install `TelemacoHttpClient` before page
...
/// Real border-box geometry for an element from the telemaco-render layout
