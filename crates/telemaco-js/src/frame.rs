...
use crate::ops::{TelemacoState, RealmStates};
...
impl FrameRealm {
    ...
    pub fn new(
        ...
        realm
            .run(
                parent,
                &format!(
                    "globalThis.__telemaco_frameId = {frame_id};\
                     globalThis.__telemaco_parentFrameId = {parent_frame_id};\
                     globalThis.__telemaco_init();"
                ),
            )
            .ok()?;
    ...
    pub fn dispatch_load_events(&self, parent: &mut TelemacoJsRuntime) -> Result<(), String> {
        self.execute_script(
            parent,
    ...
    pub fn deliver_message(
        ...
        self.execute_script(
            parent,
            &format!(
                "globalThis.__telemaco_deliverMessage({}, {}, {source_frame_id}, {});",
    ...
    fn run(&self, parent: &mut TelemacoJsRuntime, source: &str) -> Result<String, String> {
    ...
    pub fn external_script_urls(&self, parent: &mut TelemacoJsRuntime) -> Vec<String> {
    ...
    fn list_scripts(&self, parent: &mut TelemacoJsRuntime) -> Result<Vec<DocumentScript>, String> {
...
mod tests {
    ...
    fn page(url: &str, html: &str) -> TelemacoJsRuntime {
    ...
    fn frame_inherits_the_parent_browser_identity() {
        ...
        let mut parent = TelemacoJsRuntime::new();
    ...
    fn a_frame_posts_to_its_parent_as_a_trusted_message() {
        ...
        let script = format!(
            "globalThis.__telemaco_deliverMessage({}, {}, {});",
    ...
    fn post_message_honours_target_origin() {
        ...
        let deliver = |parent: &mut TelemacoJsRuntime, m: &crate::ops::PendingFrameMessage| {
            let script = format!(
                "globalThis.__telemaco_deliverMessage({}, {}, {}, {});",
    ...
    fn a_flood_of_messages_cannot_grow_the_queue_without_bound() {
        std::env::set_var("TELEMACO_FRAME_MESSAGE_QUEUE_ENTRIES", "64");
        let mut parent = page("https://parent.example/", "<html><body></body></html>");
        let frame = FrameRealm::new(
            &mut parent,
            1,
        ...
        std::env::remove_var("TELEMACO_FRAME_MESSAGE_QUEUE_ENTRIES");
    }
    ...
    fn a_discarded_realm_leaves_the_page_safe_to_run() {
        let mut parent = page("https://parent.example/", "<html><body></body></html>");
        {
            ...
            assert_eq!(
                parent
                    .evaluate("globalThis.__telemaco_frameObjects[1].window.marker")
                    .unwrap(),
                serde_json::json!("child"),
            );
        }
        ...
        parent
            .execute_script("p", "delete globalThis.__telemaco_frameObjects[1];")
            .unwrap();
        assert_eq!(
            parent
                .evaluate("globalThis.__telemaco_frameObjects[1] === undefined")
                .unwrap(),
    ...
    fn the_page_reads_the_frames_document_through_its_own_object() {
        ...
        assert_eq!(
            parent
                .evaluate("globalThis.__telemaco_frameObjects[1].document.title")
                .unwrap(),
            serde_json::json!("RAN-IN-CHILD"),
        );
        assert_eq!(
            parent
                .evaluate("globalThis.__telemaco_frameObjects[1].document.querySelector('p').textContent")
                .unwrap(),
            serde_json::json!("child"),
        );
