...
use telemaco_dom::{DomTree, NodeId};

/// Re-exported so other crates (telemaco-browser, telemaco-cdp) can name the V8
/// isolate handle without taking a direct dependency on deno_core.
pub use deno_core::v8::IsolateHandle;

use crate::import_map::ImportMap;
use crate::module_loader::{ModuleLoadActivity, TelemacoModuleLoader};
#[cfg(all(test, feature = "render"))]
use crate::ops::ensure_prepared_render;
use crate::ops::{
    build_extension, node_is_script, TelemacoState, RuntimeEvent, RuntimeExceptionEvent,
...
#[cfg(feature = "render")]
impl telemaco_render::CanvasSurfaceSource for RuntimeCanvasSurfaceSource<'_> {
    fn surface(&self, node: NodeId) -> Option<telemaco_render::CanvasSurface<'_>> {
        let surface = self.0.get(&node)?;
        telemaco_render::CanvasSurface::from_rgba8(
            surface.width,
            surface.height,
...
static SNAPSHOT: &[u8] = include_bytes!(env!("TELEMACO_SNAPSHOT_PATH"));
...
/// `TelemacoJsRuntime::new` warmup in `telemaco-cdp` server startup), which is
...
fn with_sync_render_loading_disabled<R>(
    ...
    capture: impl FnOnce(&mut TelemacoState) -> R,
) -> R {
    let previous = state
        .render_resources
...
pub struct TelemacoJsRuntime {
    runtime: JsRuntime,
    state: Rc<RefCell<TelemacoState>>,
    ...
    /// duplicate script tags and roots already seen by Telemaco.
...
/// Handle to an armed V8 execution watchdog (see [`TelemacoJsRuntime::arm_watchdog`]).
...
/// via [`TelemacoJsRuntime::cancel_termination`] before reusing the isolate.
...
impl WatchdogToken {
    ...
    /// [`TelemacoJsRuntime::cancel_termination`] before the next eval.
...
impl TelemacoJsRuntime {
    ...
    pub fn with_base_url_and_proxy(base_url: &str, proxy_url: Option<String>) -> Self {
        let state = Rc::new(RefCell::new(TelemacoState::new()));
        let state_clone = state.clone();
        let import_map = state.borrow().import_map.clone();

        let module_loader = TelemacoModuleLoader::with_page_state(
            base_url,
            proxy_url,
            &state,
            import_map.clone(),
        ...
        let (runtime, isolate_handle, heap_limit_state) = {
            ...
            runtime
                .execute_script(
                    "<telemaco:init>",
                    "globalThis.__telemaco_objects = {}; globalThis.__telemaco_oid = 0;".to_string(),
                )
        ...
        let mut instance = TelemacoJsRuntime {
            runtime,
            state,
    ...
    fn take_ops_handoff(&mut self) -> Option<deno_core::v8::Global<deno_core::v8::Value>> {
        ...
        let handoff_key = v8::String::new(scope, "__telemaco_core_handoff")?;
    ...
    pub(crate) fn share_ops_with_realm(
        ...
        let Some(handoff_key) = v8::String::new(scope, "__telemaco_core_handoff") else {
            return false;
        };
    ...
    pub(crate) fn copy_identity_to_realm(
        ...
        const IDENTITY_GLOBALS: [&str; 7] = [
            "__telemaco_ua",
            "__telemaco_platform",
            "__telemaco_ua_platform",
            "__telemaco_ua_platform_version",
            "__telemaco_stealth",
            "__telemaco_geo_lat",
            "__telemaco_geo_lon",
        ];
    ...
    pub(crate) fn share_resources_with(&self, frame: &mut TelemacoState) {
    ...
    /// page realm, under `__telemaco_frameObjects[frameId]`.
    ///
    ...
    pub(crate) fn publish_realm_objects(
        ...
        let scope = &mut v8::ContextScope::new(scope, main_context);
        let global = main_context.global(scope);
        let Some(registry_key) = v8::String::new(scope, "__telemaco_frameObjects") else {
            return false;
        };
    ...
    pub fn set_cookie_jar(&self, jar: std::sync::Arc<telemaco_net::CookieJar>) {
        self.state.borrow_mut().cookie_jar = Some(jar);
    }

    pub fn set_http_client(&self, client: std::sync::Arc<telemaco_net::TelemacoHttpClient>) {
    ...
    pub fn set_callbacks(&self, callbacks: std::sync::Arc<telemaco_net::CallbackRegistry>) {
    ...
    #[cfg(feature = "stealth")]
    pub fn set_stealth_client(&self, client: std::sync::Arc<telemaco_net::StealthHttpClient>) {
    ...
    pub fn set_dom(&self, dom: DomTree) {
        ...
        #[cfg(feature = "render")]
        {
            ...
            gs.animation_sample = telemaco_render::AnimationSample::default();
            gs.animation_timeline = telemaco_render::AnimationTimelineState::default();
            ...
            gs.render_resources = telemaco_render::RenderResourceCache::default();
            gs.render_image_in_flight.clear();
            gs.stylesheet_cache = telemaco_render::StylesheetCache::default();
        ...
        for event in &events {
            ...
            for arg in &event.args {
                ...
                let retrieval = if frame_id == 0 {
                    format!("globalThis.__telemaco_objects['{object_id}']")
                } else {
                    format!(
                        "globalThis.__telemaco_frameObjects[{frame_id}]?.window?.__telemaco_objects['{object_id}']"
                    )
                };
    ...
    pub fn set_user_agent(&mut self, ua: &str) {
        ...
        let _ = self.execute_runtime_script(
            "<set-ua>",
            format!("globalThis.__telemaco_ua = '{}';", escaped),
        );
    }
    ...
    pub fn set_platform(&mut self, platform: &str, ua_platform: &str, ua_platform_version: &str) {
        ...
        let _ = self.execute_runtime_script(
            "<set-platform>",
            format!(
                "globalThis.__telemaco_platform='{}';globalThis.__telemaco_ua_platform='{}';globalThis.__telemaco_ua_platform_version='{}';",
                p, uap, uapv
            ),
        );
    }
    ...
    pub fn set_stealth(&mut self, enabled: bool) {
        let _ = self.execute_runtime_script(
            "<set-stealth>",
            format!("globalThis.__telemaco_stealth = {};", enabled),
        );
    }
    ...
    pub fn set_viewport(&mut self, width: f64, height: f64) {
        ...
        let _ = self.execute_runtime_script(
            "<set-viewport>",
            format!(
                "globalThis.__telemaco_viewport_w={width};\
                 globalThis.__telemaco_viewport_h={height};\
                 ...
                 if(typeof globalThis.__telemaco_recompute_intersections==='function'){{\
                   globalThis.__telemaco_recompute_intersections();\
                 }}\
                 if(typeof globalThis.__telemaco_recompute_resizes==='function'){{\
                   globalThis.__telemaco_recompute_resizes();\
                 }}",
            ),
        );
    }
    ...
    pub fn set_screen_size_override(&mut self, size: Option<(f64, f64)>, emulated: bool) {
        let script = match size {
            ...
            {
                format!(
                    "globalThis.__telemaco_set_screen_override({width},{height},{emulated});"
                )
            }
            _ => format!(
                "globalThis.__telemaco_set_screen_override(null,null,{emulated});"
            ),
        };
    ...
    pub fn set_animation_sample_time(
        &self,
        sample: telemaco_render::AnimationSampleTime,
    ) -> bool {
        self.set_animation_sample(telemaco_render::AnimationSample::document(
            sample.milliseconds,
        ))
    }

    #[cfg(feature = "render")]
    pub fn set_animation_sample(&self, sample: telemaco_render::AnimationSample) -> bool {
    ...
    pub fn set_render_media(
        &self,
        media: telemaco_render::CssMediaType,
    ) -> telemaco_render::CssMediaType {
        let mut state = self.state.borrow_mut();
    ...
    #[cfg(feature = "render")]
    pub fn animation_sample_time(&self) -> telemaco_render::AnimationSampleTime {
        self.state.borrow().animation_sample.time
    }

    #[cfg(feature = "render")]
    pub fn live_animation_sample(&self) -> telemaco_render::AnimationSample {
        let state = self.state.borrow();
        telemaco_render::AnimationSample::document(
    ...
    pub fn reset_animation_timeline(&self) {
        ...
        state.animation_timeline = telemaco_render::AnimationTimelineState::default();
        state.animation_sample = telemaco_render::AnimationSample::default();
        state.prepared_render = None;
        state.pending_style_mutations.clear();
        state.resolved_scroll = None;
    }
    ...
    pub fn screenshot_unprepared_with_retained_resources(
        ...
        base_url: Option<&str>,
        scroll: (f32, f32),
        animation_sample_time: telemaco_render::AnimationSampleTime,
        surface_color: [u8; 4],
    ) -> Option<Vec<u8>> {
        let mut state = self.state.borrow_mut();
        let TelemacoState {
        ...
        telemaco_render::screenshot_png_scrolled_at_animation_time_with_surface_color_and_resources(
            dom.as_ref()?,
            viewport,
            base_url,
            scroll,
    ...
    pub fn screenshot_prepared_with_surface_color(
        ...
        with_sync_render_loading_disabled(&mut state, |state| {
            ...
            telemaco_render::screenshot_prepared_with_scroll_and_surface_color_and_canvas_surfaces(
                dom.as_ref()?,
    ...
    pub fn screenshot_prepared_region(
        &self,
        region: telemaco_render::CaptureRegion,
    ) -> Result<Vec<u8>, telemaco_render::CaptureError> {
    ...
    pub fn screenshot_prepared_region_with_surface_color(
        ...
        let mut state = self.state.borrow_mut();
        with_sync_render_loading_disabled(&mut state, |state| {
            ensure_resolved_scroll(state).ok_or(telemaco_render::CaptureError::PaintFailed)?;
            let TelemacoState {
                dom,
                prepared_render,
                render_resources,
            ...
            let (_, scroll) = resolved_scroll
                .as_ref()
                .ok_or(telemaco_render::CaptureError::PaintFailed)?;
            let canvas_surfaces = RuntimeCanvasSurfaceSource(canvas_surfaces);
            telemaco_render::screenshot_prepared_region_with_scroll_and_surface_color_and_canvas_surfaces(
                dom.as_ref()
                    .ok_or(telemaco_render::CaptureError::PaintFailed)?,
                prepared_render
                    .as_mut()
    ...
    pub fn screenshot_prepared_region_with_backgrounds(
        ...
        with_sync_render_loading_disabled(&mut state, |state| {
            ...
            let (_, scroll) = resolved_scroll
                ...
                .ok_or(telemaco_render::CaptureError::PaintFailed)?;
            let canvas_surfaces = RuntimeCanvasSurfaceSource(canvas_surfaces);
            telemaco_render::screenshot_prepared_region_with_scroll_and_backgrounds_and_canvas_surfaces(
                dom.as_ref()
                    .ok_or(telemaco_render::CaptureError::PaintFailed)?,
                prepared_render
                    .as_mut()
    ...
    pub fn seed_render_image_resource(
        ...
        let mut state = self.state.borrow_mut();
        match bytes {
            Some(bytes) if telemaco_render::image_intrinsic_dimensions(&bytes).is_some() => {
    ...
    /// Run __telemaco_init() after all per-page properties (UA, platform, stealth, etc.)
    /// have been set. Must be called once per page setup, after all set_* methods.
    pub fn run_page_init(&mut self) {
        let _ = self.execute_runtime_script(
            "<telemaco:page-init>",
            "globalThis.__telemaco_init();".to_string(),
        );
    }
    ...
    pub fn set_geolocation(&mut self, latitude: f64, longitude: f64) {
        let _ = self.execute_runtime_script(
            "<set-geo>",
            format!(
                "globalThis.__telemaco_geo_lat={};globalThis.__telemaco_geo_lon={};",
                latitude, longitude
            ),
        );
    }
    ...
    pub async fn evaluate_for_cdp_with_timeout(
        ...
        let meta_code = if await_promise {
            format!(
                "(async function() {{\n\
                    try {{\n\
                        var __result = await (\n{expr}\n);\n\
                        globalThis.__telemaco_objects['{oid}'] = __result;\n\
                        globalThis.__telemaco_await_meta = {meta_fn};\n\
                        globalThis.__telemaco_await_rejected = false;\n\
                    }} catch(e) {{\n\
                        globalThis.__telemaco_objects['{oid}'] = e;\n\
                        globalThis.__telemaco_await_meta = {err_meta_fn};\n\
                        globalThis.__telemaco_await_rejected = true;\n\
                    }}\n\
                    globalThis.__telemaco_done_{done_counter} = true;\n\
                }})()",
                expr = cleaned_expr,
                oid = oid,
        ...
        let meta_str = if await_promise {
            ...
            let sentinel = format!("globalThis.__telemaco_done_{done_counter} === true");
            let settled = self
            ...
            let rejected = self
                .execute_runtime_script(
                    "<readRejected>",
                    "globalThis.__telemaco_await_rejected".to_string(),
                )
                .map_err(|e| format!("JS error: {}", e))?;
            if self.v8_to_json(rejected)?.as_bool().unwrap_or(false) {
                let err = self.execute_runtime_script("<readError>", format!("String(globalThis.__telemaco_objects['{0}'] && (globalThis.__telemaco_objects['{0}'].message || globalThis.__telemaco_objects['{0}']))", oid))
                    .map_err(|e| format!("JS error: {}", e))?;
                return Err(format!(
                    "Promise rejected: {}",
            ...
            self.execute_runtime_script("<readMeta>", "globalThis.__telemaco_await_meta".to_string())
                .map_err(|e| format!("JS error: {}", e))?
        } else {
            result
        };
        ...
        self.object_store.insert(
            oid.clone(),
            format!("globalThis.__telemaco_objects['{}']", oid),
        );
    ...
    pub async fn call_function_on_for_cdp_with_timeout(
        ...
        if await_promise {
            ...
            let meta_result = self
                .execute_runtime_script("<readMeta>", "globalThis.__telemaco_await_meta".to_string())
                .map_err(|e| format!("JS error: {}", e))?;
    ...
    pub fn release_object(&mut self, object_id: &str) {
        if self.object_store.remove(object_id).is_some() {
            ...
            let code = if frame_id == 0 {
                format!("delete globalThis.__telemaco_objects['{object_id}'];")
            } else {
                format!(
                    "delete globalThis.__telemaco_frameObjects[{frame_id}]?.window?.__telemaco_objects['{object_id}'];"
                )
            };
    ...
    pub fn release_object_group(&mut self) {
        ...
        frame_ids.dedup();
        let mut code = "globalThis.__telemaco_objects = {};".to_string();
        for frame_id in frame_ids {
            code.push_str(&format!(
                "if(globalThis.__telemaco_frameObjects[{frame_id}]?.window)globalThis.__telemaco_frameObjects[{frame_id}].window.__telemaco_objects={{}};"
            ));
        }
    ...
    async fn drive_module_eval(
        ...
        // covers duplicate roots prepared by Telemaco. A root can also have
    ...
    pub fn has_pending_dynamic_scripts(&mut self) -> bool {
        let pending_dom_script = self
            .evaluate("globalThis.__telemaco_hasPendingDynamicScripts?.() === true")
            .ok()
    ...
    pub fn has_pending_load_delaying_scripts(&mut self) -> bool {
        self.evaluate("globalThis.__telemaco_hasPendingLoadDelayingScripts?.() === true")
            .ok()
            .and_then(|value| value.as_bool())
            .unwrap_or(false)
    }
    ...
    fn next_pending_timeout_delay_ms(&mut self) -> Option<f64> {
        self.evaluate("globalThis.__telemaco_nextPendingTimeoutDelay?.() ?? -1")
        .ok()
    ...
    pub fn take_dom(&self) -> Option<DomTree> {
        ...
        #[cfg(feature = "render")]
        {
            state.prepared_render = None;
            state.pending_style_mutations.clear();
            state.render_resources = telemaco_render::RenderResourceCache::default();
            state.stylesheet_cache = telemaco_render::StylesheetCache::default();
...
impl Default for TelemacoJsRuntime {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn setup_runtime(html: &str) -> TelemacoJsRuntime {
        let dom = parse_html(html);
        let mut rt = TelemacoJsRuntime::new();
        rt.set_dom(dom);
    ...
    fn document_domain_detached_and_hostless_setters_throw_security_error() {
        ...
        let dom = parse_html("<html><body></body></html>");
        let mut hostless = TelemacoJsRuntime::new();
    ...
    fn time_origin_never_lands_in_the_future() {
        // __telemaco_init deletes itself, so a realm yields one draw of the
        ...
        let result = rt
            .evaluate(
                ...
                return [
                    ...
                    matchMedia("(telemaco-unknown-feature: yes)").matches
                ];
                "#,
            )
            .unwrap();
    ...
    async fn quiescent_event_loop_retains_delayed_network_and_dom_update() {
        let mut rt = setup_runtime("<html><body></body></html>");
        let client = std::sync::Arc::new(telemaco_net::TelemacoHttpClient::with_full_options(
            std::sync::Arc::new(telemaco_net::CookieJar::new()),
            None,
            true,
        ));
    ...
    fn delayed_fetch_runtime(
    ...
    ) -> (TelemacoJsRuntime, std::sync::mpsc::Receiver<()>) {
        ...
        rt.set_http_client(std::sync::Arc::new(
            telemaco_net::TelemacoHttpClient::with_full_options(
                std::sync::Arc::new(telemaco_net::CookieJar::new()),
                None,
                true,
            ),
        ...
        let result = rt
            .evaluate(
                r#"JSON.stringify([
                    ...
                    CSS.supports("selector(:telemaco-unknown)"),
                    CSS.supports("selector(.card,)"),
    ...
    fn fractional_scroll_ranges_quantize_geometry_and_pixels_at_one_x() {
        ...
        let mut root_rt = TelemacoJsRuntime::new();
    ...
    fn element_scroll_offsets_follow_chromium_box_and_dom_lifecycles() {
        ...
        let result = rt
            .evaluate(
                ...
                let recomputes = 0;
                globalThis.__telemaco_recompute_intersections = () => { recomputes++; };
    ...
    fn prepared_render_shares_resource_geometry_with_cssom_and_screenshots() {
        ...
        rt.state.borrow_mut().render_resources =
            telemaco_render::RenderResourceCache::with_loader(move |url: &str| {
        ...
        let (prepared_address, prepared_height) = {
            ...
            (
                prepared as *const telemaco_render::PreparedRender as usize,
        ...
        assert_eq!(bottom, bottom_repeat);
        {
            ...
            assert_eq!(
                prepared as *const telemaco_render::PreparedRender as usize, prepared_address,
    ...
    async fn missing_render_resource_preserves_prepared_layout_and_scroll() {
        ...
        let (prepared_address, resolved_address, scroll_generation, root_offset) = {
            ...
            (
                prepared as *const telemaco_render::PreparedRender as usize,
                resolved as *const telemaco_render::ResolvedScrollState as usize,
        ...
        assert!(rt.render_resource_is_known(&missing_url));
        {
            ...
            assert_eq!(
                resolved as *const telemaco_render::ResolvedScrollState as usize, resolved_address,
        ...
        assert_eq!(
            ...
            vec![telemaco_render::RetainedStyleMutation::Resource],
    ...
    fn image_resource_arrival_retains_styles_and_rebuilds_intrinsic_geometry() {
        ...
        rt.set_viewport(80.0, 60.0);
        rt.state.borrow_mut().render_resources =
            telemaco_render::RenderResourceCache::with_loader(|_: &str| None);
        rt.run_page_init();
        ...
        let prepared_address = {
            ...
            state
                .prepared_render
                .as_ref()
                .expect("initial prepared render") as *const telemaco_render::PreparedRender
                as usize
        };
        ...
        );
        {
            ...
            assert_eq!(
                state
                    ...
                    as *const telemaco_render::PreparedRender as usize,
                prepared_address,
            );
            assert_eq!(
                state
                    ...
                    .iter()
                    .filter(|mutation| matches!(mutation, telemaco_render::RetainedStyleMutation::Resource))
                    .count(),
                1,
            );
            assert!(state.pending_style_mutations.iter().any(|mutation| matches!(
                mutation,
                telemaco_render::RetainedStyleMutation::Attribute(_)
            )));
        }
    ...
    fn document_region_capture_preserves_live_runtime_state_and_resource_cache() {
        ...
        let region_png = rt
            .screenshot_prepared_region(telemaco_render::CaptureRegion::new(
    ...
    fn script_registered_url_font_reaches_render_resource_collection() {
        ...
        let loader_loads = std::sync::Arc::clone(&loads);
        let font = include_bytes!("../../telemaco-render/assets/liberation-serif.ttf").to_vec();
        ...
        .unwrap();
        {
            ...
            assert!(matches!(
                ...
                [telemaco_render::RetainedStyleMutation::Attribute(
                    telemaco_render::AttributeStyleMutation { name, .. }
                )] if name == "style"
            ));
        }
        assert_eq!(
    ...
    fn forward_animation_samples_retain_static_prepared_render() {
        ...
        assert!(rt.set_animation_sample_time(telemaco_render::AnimationSampleTime {
            milliseconds: 100.0,
        }));
        let first = rt
            .screenshot_prepared((80.0, 60.0), Some("http://example.test/page"))
    ...
    fn forward_active_animation_sample_updates_geometry_and_paint_from_retained_frame() {
        ...
        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::document(0.0)));
        let initial = rt
        ...
        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::document(500.0)));
        assert!(
            rt.state.borrow().prepared_render.is_some(),
    ...
    fn forward_waapi_sample_updates_retained_style_and_paint() {
        ...
        .unwrap();
        {
            ...
            assert_eq!(
                state.pending_style_mutations,
                vec![telemaco_render::RetainedStyleMutation::WaapiAnimation {
                    node: box_node
                }]
            );
        }
    ...
    fn remove_and_reappend_restarts_animation_without_intermediate_flush() {
        ...
        rt.evaluate("var box=document.createElement('div');box.id='box';box.className='anim';document.body.appendChild(box)")
            .unwrap();
        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::document(1_000.0)));
        ...
        rt.evaluate("var box=document.getElementById('box');box.remove();document.body.appendChild(box)")
            .unwrap();
        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::document(1_100.0)));
        rt.screenshot_prepared((200.0, 80.0), Some("http://example.test/page"))
            .unwrap();
    ...
    fn scoped_animation_epochs_survive_later_unrelated_mutations_and_t0_capture() {
        ...
        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::local_override(0.0)));
        rt.screenshot_prepared((200.0, 80.0), Some("http://example.test/page"))
            .unwrap();
        assert_eq!(animation_test_width(&rt, "first"), 0.0);
        assert_eq!(animation_test_width(&rt, "second"), 0.0);

        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::document(700.0)));
        rt.screenshot_prepared((200.0, 80.0), Some("http://example.test/page"))
            .unwrap();
    ...
    fn timing_edits_preserve_identity_and_pause_holds_then_resumes() {
        ...
        rt.evaluate("var box=document.createElement('div');box.id='box';box.className='anim';document.body.appendChild(box)")
            .unwrap();
        assert!(rt.set_animation_sample(telemaco_render::AnimationSample::document(300.0)));
        rt.screenshot_prepared((200.0, 80.0), Some("http://example.test/page"))
            .unwrap();
    ...
    fn fixed_animation_capture_is_invariant_after_geometry_flush() {
        ...
        let fixed = telemaco_render::AnimationSample::local_override(750.0);
    ...
    fn autocomplete_attribute_retains_prepared_render_until_geometry_flush() {
            ...
            .unwrap();
        {
            ...
            assert!(matches!(
                state.pending_style_mutations.as_slice(),
                [telemaco_render::RetainedStyleMutation::Attribute(
                    telemaco_render::AttributeStyleMutation { name, old_value, new_value, .. }
    ...
    fn connected_tree_mutations_queue_retained_styles_until_geometry_flush() {
        ...
        .unwrap();
        {
            ...
            assert!(matches!(
                state.pending_style_mutations.as_slice(),
                [telemaco_render::RetainedStyleMutation::Tree(
        ...
        .unwrap();
        {
            ...
            assert!(matches!(
                state.pending_style_mutations.as_slice(),
                [telemaco_render::RetainedStyleMutation::Tree(
                    telemaco_render::TreeStyleMutation::Remove { .. }
                )]
            ));
        }
        assert_eq!(
    ...
    async fn resize_observer_reports_real_boxes_only_when_selected_size_changes() {
        ...
        assert_eq!(
            rt.evaluate("__telemaco_nextPendingTimeoutDelay()")
                .unwrap()
                .as_f64(),
            Some(-1.0)
        );
    ...
    async fn resize_observer_self_resize_is_depth_bounded_without_timer_spin() {
        ...
        assert_eq!(
            rt.evaluate(
                "[__resizeCallbacks, __resizeLoopErrors, __telemaco_nextPendingTimeoutDelay()]"
            )
            .unwrap(),
        ...
        let result = rt
            .evaluate(
                ...
                (() => {
                    ...
                    const node = document.querySelector('style[data-telemaco-adopted]');
                    ...
                    return [
                        ...
                        document.querySelectorAll('style[data-telemaco-adopted]').length,
        ...
        let result = rt
            .evaluate(
                ...
                (() => {
                    ...
                    const firstNode = first.querySelector('style[data-telemaco-adopted]');
                    const secondNode = second.querySelector('style[data-telemaco-adopted]');
                    const documentNode = document.querySelector('style[data-telemaco-adopted]');
                    const initial = [
                    ...
                    const afterRemoval = [
                        ...
                        second.querySelectorAll('style[data-telemaco-adopted]').length,
    ...
    fn canvas_2d_live_backing_paints_immediately_with_scaling_clips_and_effects() {
        ...
        let pixmap = {
            ...
            let canvas_surfaces = RuntimeCanvasSurfaceSource(canvas_surfaces);
            telemaco_render::paint_prepared_with_scroll_and_surface_color_and_canvas_surfaces(
    ...
    fn dynamic_script_status_bridge_is_hidden_and_idle() {
        ...
        assert_eq!(
            rt.evaluate(
                "Object.getOwnPropertyNames(globalThis).includes('__telemaco_hasPendingDynamicScripts')"
            )
            .unwrap(),
        ...
        assert_eq!(
            rt.evaluate(
                "Reflect.ownKeys(globalThis).includes('__telemaco_hasPendingDynamicScripts')"
            )
            .unwrap(),
            serde_json::json!(false)
        );
        assert_eq!(
            rt.evaluate(
                "Reflect.ownKeys(globalThis).includes('__telemaco_hasPendingLoadDelayingScripts')"
            )
            .unwrap(),
            serde_json::json!(false)
        );
        assert_eq!(
            rt.evaluate(
                "Object.getOwnPropertyNames(globalThis).includes('__telemaco_nextPendingTimeoutDelay')"
            )
            .unwrap(),
            serde_json::json!(false)
        );
        assert_eq!(
            rt.evaluate(
                "Reflect.ownKeys(globalThis).includes('__telemaco_nextPendingTimeoutDelay')"
            )
            .unwrap(),
            serde_json::json!(false)
        );
    ...
    fn parser_image_runtime(
        html: &str,
        loader: impl telemaco_render::RenderResourceLoader + 'static,
    ) -> TelemacoJsRuntime {
        let mut rt = TelemacoJsRuntime::new();
        rt.set_dom(parse_html(html));
        rt.set_url("http://example.com/page/index.html");
        rt.state.borrow_mut().render_resources =
            telemaco_render::RenderResourceCache::with_loader(loader);
        rt.run_page_init();
        rt
    }
    ...
    async fn stable_cached_image_getters_do_not_queue_resize_geometry_work() {
        ...
        assert_eq!(
            rt.evaluate(
                "[__stableGetterResizeRecords, __telemaco_nextPendingTimeoutDelay()]"
            )
            .unwrap(),
            serde_json::json!([1, -1])
        );
    ...
    #[cfg(feature = "render")]
    fn change_srcset_image_sizes(rt: &mut TelemacoJsRuntime) {
    ...
    fn test_sequential_runtime_swap() {
        ...
        if let Some(dom) = dom1 {
            let mut rt1b = TelemacoJsRuntime::new();
            rt1b.set_dom(dom);
    ...
    // controlled write. __telemaco_setFieldValue must write through the prototype
        ...
        let result = rt
            .evaluate(
                ...
                (function(){
                    ...
                    globalThis.__telemaco_setFieldValue(el, 'value', 'native');
    ...
    fn setup_runtime_with_cookies(
        html: &str,
    ) -> (TelemacoJsRuntime, std::sync::Arc<telemaco_net::CookieJar>) {
        let dom = telemaco_dom::parse_html(html);
        let jar = std::sync::Arc::new(telemaco_net::CookieJar::new());
        let mut rt = TelemacoJsRuntime::new();
        rt.set_dom(dom);
        rt.set_url("http://example.com/test");
        rt.set_title("Test Page");
    ...
    fn setup_runtime_at_deep_url(html: &str) -> TelemacoJsRuntime {
        let dom = parse_html(html);
        let mut rt = TelemacoJsRuntime::new();
        rt.set_dom(dom);
    ...
    fn redirect_chain_runtime(connections: usize) -> TelemacoJsRuntime {
        let listener = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let address = listener.local_addr().unwrap();
        std::thread::spawn(move || {
            use std::io::{Read as _, Write as _};
            ...
            .call_function_on_for_cdp(
                r#"async () => {
                    ...
                    try {
                        ...
                        await loaded;
                        const style = document.querySelector("style[data-telemaco-linked]");
                        ...
                        return {
                            ...
                            removedWithLink:
                                !document.querySelector("style[data-telemaco-linked]"),
                            cssom,
                ...
                r#"async () => {
                    ...
                    try {
                        ...
                        return {
                            asyncOrder,
                            inOrder,
                            pending: globalThis.__telemaco_hasPendingDynamicScripts(),
                        };
                    } finally {
                        Deno.core.ops.op_fetch_url = originalFetchOp;
                    }
    ...
    async fn entry_module_http_failure_is_not_evaluated_as_empty_source() {
        ...
        let mut rt = TelemacoJsRuntime::with_base_url(&format!("{}/", base));
        rt.set_http_client(client);
    ...
    async fn descendant_module_uses_page_cookie_identity_and_headers() {
        ...
        let callbacks = std::sync::Arc::new(telemaco_net::CallbackRegistry::new());
    ...
    async fn cross_origin_module_descendant_does_not_gain_module_origin_cookies() {
        ...
        let mut rt = TelemacoJsRuntime::with_base_url(document_url);
        rt.set_http_client(client);
    ...
    async fn import_map_resolves_prefix_static_and_exact_dynamic_imports() {
        ...
        let mut rt = TelemacoJsRuntime::with_base_url(&format!("{}/app/index.html", base));
        rt.set_http_client(client);
        rt.add_import_map(
            r#"{
                "imports": {
    ...
    async fn import_map_does_not_remap_external_root_module_url() {
        ...
        let mut rt = TelemacoJsRuntime::with_base_url(&format!("{}/index.html", base));
        rt.set_http_client(client);
        rt.add_import_map(
    ...
    async fn inline_modules_expose_document_base_as_import_meta_url() {
        let mut rt = TelemacoJsRuntime::with_base_url("https://example.com/page/index.html");
        rt.load_inline_module(
    ...
    async fn classic_script_url_is_dynamic_import_referrer() {
        ...
        let mut rt = TelemacoJsRuntime::with_base_url(&format!("{base}/page/index.html"));
        rt.set_http_client(client);
        rt.set_dom(parse_html("<html><body></body></html>"));
        rt.run_page_init();
        rt.add_import_map(
    ...
    async fn inline_module_graph_error_propagates() {
        let mut rt = TelemacoJsRuntime::with_base_url("https://example.com/");
        let error = rt
    ...
    fn http_client_round_trips_proxy_url() {
        use telemaco_net::{CookieJar, TelemacoHttpClient};
        let jar = std::sync::Arc::new(CookieJar::new());
        let configured =
            TelemacoHttpClient::with_options(jar.clone(), Some("http://proxy.test:8080"));
        assert_eq!(
        ...
        let direct = TelemacoHttpClient::with_options(jar, None);
        assert_eq!(
            direct.proxy_url(),
            None,
    ...
    fn module_loader_stores_proxy_for_dynamic_imports() {
        use crate::module_loader::TelemacoModuleLoader;
        let loader = TelemacoModuleLoader::with_proxy(
            "https://example.com/",
        ...
        let direct = TelemacoModuleLoader::new("https://example.com/");
    ...
    fn runtime_with_base_url_and_proxy_constructs_successfully() {
        ...
        let _direct = TelemacoJsRuntime::with_base_url_and_proxy("https://example.com/", None);
        let _proxied = TelemacoJsRuntime::with_base_url_and_proxy(
            "https://example.com/",
            Some("http://proxy.test:8080".to_string()),
        );
    }
