...
fn main() {
    ...
    let snapshot_path = out_dir.join("TELEMACO_SNAPSHOT.bin");
    ...
    if is_cross {
        ...
        eprintln!("[telemaco-js build.rs] Cross-compilation detected (host={host}, target={target})");
        eprintln!("[telemaco-js build.rs] Using cross-mksnapshot for target-compatible snapshot");

        // Locate the cross-mksnapshot binary.
        // OUT_DIR is under target/<triple>/<profile>/build/telemaco-js-*/out,
        // so the GN output directory is 4 levels up.
        let target_dir = out_dir
            .parent() // build/telemaco-js-*/
        ...
        let cross_mksnapshot = match cross_mksnapshot {
            Some(p) => {
                eprintln!("[telemaco-js build.rs] Found cross-mksnapshot: {}", p.display());
                p
            }
            None => {
                // Fallback: use host V8 (will produce wrong-arch snapshot).
                eprintln!("[telemaco-js build.rs] WARNING: cross-mksnapshot not found, falling back to host V8 snapshot");
                eprintln!("[telemaco-js build.rs] The snapshot may crash on the target device!");
        ...
        eprintln!("[telemaco-js build.rs] Extracting deno_core sidecar from host snapshot...");
        let host_output = deno_core::snapshot::create_snapshot(
            deno_core::snapshot::CreateSnapshotOptions {
                ...
                with_runtime_cb: Some(Box::new(move |runtime| {
                    runtime
                        .execute_script("<telemaco:bootstrap>", bootstrap_js.to_string())
        ...
        eprintln!("[telemaco-js build.rs] Running cross-mksnapshot for target architecture...");
        ...
        if raw_version != expected_version {
            eprintln!(
                "[telemaco-js build.rs] Patching snapshot version: {} -> {}",
        ...
        eprintln!(
            "[telemaco-js build.rs] Cross-compiled snapshot written: {} bytes (v8_blob={}, sidecar={})",
            combined.len(),
            raw_blob.len(),
            sidecar.len()
        );
        ...
        println!(
            "cargo:rustc-env=TELEMACO_SNAPSHOT_PATH={}",
