...
(function _preHideInternals() {
  var _names = [
    ...
    '__telemaco_errors', '__telemaco_init', '__telemaco_hide_list',
    '__telemaco_objects', '__telemaco_oid', '__telemaco_ua',
    '__telemaco_platform', '__telemaco_ua_platform', '__telemaco_ua_platform_version',
    '__telemaco_stealth', '__telemaco_markTrusted', '__telemaco_core_handoff',
    '__telemaco_frameId', '__telemaco_parentFrameId', '__telemaco_frameWindows',
    '__telemaco_frameObjects', '__telemaco_frameElements', '__telemaco_deliverMessage',
    '__telemaco_liveFrameIds', '__telemaco_forgetFrame',
    '__telemaco_registerLinkedStylesheet', '__telemaco_activateLabel',
    '__telemaco_isDisabled', '__telemaco_labeledControl', '__telemaco_interactiveHost',
    '__markParserScripts', '__telemaco_hasPendingDynamicScripts',
    '__telemaco_hasPendingLoadDelayingScripts',
    '__telemaco_nextPendingTimeoutDelay',
    '__telemaco_hw', '__telemaco_mem',
...
globalThis.__telemaco_core_handoff = Deno.core;

globalThis.__telemaco_errors = [];
...
globalThis.onerror = function(msg, src, line, col, error) {
  globalThis.__telemaco_errors.push({msg: String(msg), src: String(src||""), line, error: String(error||"")});
};
...
// the caller's. Set by __telemaco_init; 0 is the page.
let _realmFrameId = 0;
...
// unusualWindowProperties: telemaco's internal globals are made non-enumerable
// (see _preHideInternals and __telemaco_init), which hides them from
...
// __telemaco_hide_list, precomputed at snapshot-build time; referencing it lazily
...
(function _hideInternalsFromReflection() {
  ...
  function _set() {
    var list = globalThis.__telemaco_hide_list;
    if (!list) { return null; }
    if (_cache && _cacheLen === list.length) { return _cache; }
    _cache = new Set(list);
    _cache.add('__telemaco_hide_list');
...
Object.defineProperty(globalThis, '__telemaco_hasPendingDynamicScripts', {
  value: function() {
...
Object.defineProperty(globalThis, '__telemaco_hasPendingLoadDelayingScripts', {
...
globalThis.__telemaco_registerLinkedStylesheet = _registerLinkedStylesheet;
...
async function _loadLinkedStylesheet(c) {
  // telemaco does not yet reflect the `rel` IDL attribute back to the content
  ...
  try {
    ...
    style.setAttribute("data-telemaco-linked", fullUrl);
...
function _getFp() {
  ...
  const _uaPlat = globalThis.__telemaco_ua_platform || 'Windows';
...
const _consoleObjectId = (value) => {
  const objectId = "console-" + (globalThis.__telemaco_frameId >>> 0) + "-" + (++_consoleOid);
  const store = globalThis.__telemaco_objects || (globalThis.__telemaco_objects = {});
...
const __telemacoPendingTimeoutDeadlines = new Map();
Object.defineProperty(globalThis, '__telemaco_nextPendingTimeoutDelay', {
  value: function() {
    const now = performance.now();
    let nearest = Infinity;
    for (const deadline of __telemacoPendingTimeoutDeadlines.values()) {
...
const _scheduleAfter = (delay, fn) => {
  ...
  // permanent set.
  if (globalThis.__telemaco_frameId) {
...
globalThis.setTimeout = (fn, delay = 0, ...args) => {
  ...
  const nativeId = _scheduleAfter(normalizedDelay, () => {
    ...
    __telemacoPendingTimeoutDeadlines.delete(id);
    if (state.cancelled) return;
  ...
  if (nativeId !== undefined) {
    ...
    __telemacoPendingTimeoutDeadlines.set(id, performance.now() + normalizedDelay);
  }
  return id;
};
...
// non-enumerable __telemaco_activateLabel helper, so both click paths apply the
// same rule.
...
// __telemaco_activateLabel below will supply it on request, exactly as
// __telemaco_markTrusted already does for any event.
const _TRUSTED_ACTIVATION = Symbol('telemaco.trustedActivation');
...
globalThis.__telemaco_activateLabel = function(label, control, trusted) {
...
globalThis.__telemaco_isDisabled = function(el) { return _isActuallyDisabled(el); };
globalThis.__telemaco_labeledControl = function(label) { return _labeledControl(label); };
...
class Element extends Node {
  ...
  click() {
    ...
    // telemaco-cdp/src/domains/input.rs, which already implements it; without it
    ...
    if (_trusted) globalThis.__telemaco_markTrusted(_clickEvent);
    ...
    if (_checkable && this.checked !== _oldChecked) {
      for (const _type of ['input', 'change']) {
        ...
        if (_trusted) globalThis.__telemaco_markTrusted(_e);
    ...
    if (_label && !(this.closest && this.closest(_INTERACTIVE) &&
      ...
      if (control && control !== this && globalThis.__telemaco_activateLabel(_label, control)) {
        return;
      }
    }
    if (!cancelled) {
  ...
  // the DOM. See __telemaco_setInputFiles (issue #359).
  get files() {
  ...
  _resetIframeFrame() {
    ...
    if (oldId) {
      delete globalThis.__telemaco_frameElements[oldId];
      delete globalThis.__telemaco_frameWindows[oldId];
    }
    this._frameId = 0;
  ...
  _loadIframeSrc(url) {
    ...
    // Both the src setter and the parser sweep in __telemaco_init reach here, so
    ...
    fetch(fullUrl, {mode: 'no-cors'}).then(async resp => {
      ...
      if (resp.ok || resp.type === 'opaque') {
        ...
        if (el._frameId) globalThis.__telemaco_frameElements[el._frameId] = el;
        ...
        if (el._frameId) {
          ...
          globalThis.__telemaco_frameWindows[el._frameId] = el._iframeWin;
          globalThis.__telemaco_frameElements[el._frameId] = el;
        }
      } else {
  ...
  // deliberately retains Telemaco's compatibility geometry.
  _renderBoxGeometry() {
  ...
  _rectFromRenderGeometry(geometry) {
    ...
    Object.defineProperty(rect, "__telemacoViewportFixed", {
  ...
  set scrollTop(v) {
    ...
    if (changed &&
        typeof globalThis.__telemaco_recompute_intersections === "function") {
  ...
  scrollIntoView(arg) {
    ...
    if (rect.__telemacoViewportFixed) return;
...
// Document in __telemaco_init) naturally restores the URL host.  Detached
...
class Document extends Node {
  ...
  get activeElement() { return globalThis.__telemaco_focused || this.body; }
...
function _chromeMajor() {
  var m = (globalThis.__telemaco_ua || '').match(/Chrome\/(\d+)/);
  return m ? (m[1] | 0) : 145;
}
...
globalThis.navigator = {
  ...
  userAgentData: {
    ...
    get platform() { return globalThis.__telemaco_ua_platform || "Windows"; },
    getHighEntropyValues(hints) {
      ...
      return Promise.resolve({
        ...
        mobile: false,
        model: "",
        platform: globalThis.__telemaco_ua_platform || "Windows",
        platformVersion: globalThis.__telemaco_ua_platform_version || "15.0.0",
  ...
  geolocation: {
    getCurrentPosition(success, error) {
      const coords = {
        latitude: (globalThis.__telemaco_geo_lat ?? 50.1109) + (_fpRand(500) - 0.5) * 0.1,
        longitude: (globalThis.__telemaco_geo_lon ?? 8.6821) + (_fpRand(501) - 0.5) * 0.1,
    ...
    watchPosition(success, error) {
      if (typeof success === 'function') {
        const coords = {
          latitude: (globalThis.__telemaco_geo_lat ?? 50.1109) + (_fpRand(503) - 0.5) * 0.1,
          longitude: (globalThis.__telemaco_geo_lon ?? 8.6821) + (_fpRand(504) - 0.5) * 0.1,
...
// Getters read __telemaco_* lazily (snapshot vs per-page) and are _markNative'd.
(function() {
  ...
  defGetter('userAgent', function() {
    return globalThis.__telemaco_ua ||
  ...
  defGetter('platform', function() {
    return globalThis.__telemaco_platform || "Win32";
  });
  ...
  // Values set per-page by __telemaco_init (avoids own data props on navigator).
  defGetter('hardwareConcurrency', function() { return globalThis.__telemaco_hw || 8; });
  defGetter('deviceMemory', function() { return globalThis.__telemaco_mem || 8; });
...
globalThis.__telemaco_set_screen_override = function(w, h, emulated) {
  globalThis.__telemaco_screen_emulated = !!emulated;
  if (Number.isFinite(w) && Number.isFinite(h) && w > 0 && h > 0) {
    globalThis.__telemaco_screen_w = w;
    globalThis.__telemaco_screen_h = h;
    _applyScreenSize(w, h, !!emulated);
    return;
  }
  delete globalThis.__telemaco_screen_w;
  delete globalThis.__telemaco_screen_h;
...
function _bodyToUint8Array(body) {
  ...
  // telemaco's Blob materializes its data into _bytes in the constructor.
...
function _installWasmStreamingFallback() {
  ...
  if (WebAssembly.instantiateStreaming && WebAssembly.instantiateStreaming.__telemacoFallback) return;
  ...
  fallback.__telemacoFallback = true;
...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (parsed.requestId) {
    Object.defineProperty(response, "__telemacoRequestId", {
...
if (typeof URL === 'undefined' || !URL.prototype || !URL.__telemaco) {
...
globalThis.__telemaco_recompute_resizes = _scheduleResizeRenderCheckpoint;
...
function _styleElementIsCssomBridge(style) {
  return style.hasAttribute("data-telemaco-adopted")
    || style.hasAttribute("data-telemaco-linked")
    || style.hasAttribute("data-telemaco-external-stylesheets")
    || style.hasAttribute("data-telemaco-inline-import");
}
...
function _syncAdoptedStyles(root) {
  ...
  for (const sheet of sheets) {
    ...
    if (!node || node.parentNode !== target) {
      ...
      node.setAttribute("data-telemaco-adopted", "");
...
globalThis.__notifyMutation = function(type, target_nid, addedNodes, removedNodes, attributeName, oldValue) {
  ...
  // MutationObserver fired in telemaco, ever, despite the call sites being
...
globalThis.__telemaco_shadowHostNames = new Set(['article','aside','blockquote','body','div','footer','h1','h2','h3','h4','h5','h6','header','main','nav','p','section','span']);
...
(function() {
  ...
  globalThis.__telemaco_recompute_intersections = _scheduleIntersectionRenderCheckpoint;
...
// telemaco's CDP input pipeline marks its synthetic events via the
// non-enumerable __telemaco_markTrusted helper.
const _trustedEvents = new WeakSet();
globalThis.__telemaco_markTrusted = function(ev) { try { if (ev) _trustedEvents.add(ev); } catch (_e) {} return ev; };
...
// direct assignment.
globalThis.__telemaco_setFieldValue = function(el, field, value) {
  try {
...
globalThis.__telemaco_setInputFiles = function(el, specs) {
  ...
  try { el.dispatchEvent(globalThis.__telemaco_markTrusted(new Event("input", { bubbles: true }))); } catch (_e) {}
  try { el.dispatchEvent(globalThis.__telemaco_markTrusted(new Event("change", { bubbles: true }))); } catch (_e) {}
};
...
// Conservative XML well-formedness check. telemaco has no XML parser, so this
...
globalThis.DOMParser = class DOMParser {
  parseFromString(source, mimeType) {
    ...
    // Chrome. telemaco has no XML parser, so the tree stays HTML-parsed.
...
function _structuredClone(value, seen) {
  ...
  if (typeof value[Symbol.toStringTag] === "string" && globalThis.__telemaco_clone_hooks) {
    const hook = globalThis.__telemaco_clone_hooks[value[Symbol.toStringTag]];
...
function _cssSupportsColor(value) {
  ...
  if (/^rgba?\(/.test(lower) && lower.endsWith(")")) {
    ...
    // Telemaco yet and therefore must not select an unsupported @supports arm.
...
// True when the document was loaded from an XML/XHTML source. Telemaco has no
...
globalThis.__telemaco_frameId = 0;        // 0 is the page's own realm
globalThis.__telemaco_parentFrameId = 0;
globalThis.__telemaco_frameWindows = Object.create(null); // frame id -> its window
...
globalThis.__telemaco_frameElements = Object.create(null);
...
globalThis.__telemaco_frameObjects = Object.create(null);
...
globalThis.__telemaco_liveFrameIds = function () {
  const live = [];
  for (const id in globalThis.__telemaco_frameElements) {
    const element = globalThis.__telemaco_frameElements[id];
...
globalThis.__telemaco_forgetFrame = function (frameId) {
  delete globalThis.__telemaco_frameElements[frameId];
  delete globalThis.__telemaco_frameObjects[frameId];
  delete globalThis.__telemaco_frameWindows[frameId];
};
...
function _sendRealmMessage(targetFrameId, data, targetOrigin) {
  ...
  // enforces a specified one against its own origin in __telemaco_deliverMessage.
  const to = (targetOrigin === undefined || targetOrigin === null) ? '' : String(targetOrigin);
  Deno.core.ops.op_post_frame_message(
    targetFrameId >>> 0, globalThis.__telemaco_frameId >>> 0, _realmOrigin(), to, json);
}
...
function _frameObjectsFor(element) {
  ...
  const entry = globalThis.__telemaco_frameObjects[frameId];
  return entry || null;
}
...
function _frameWindowFor(frameId) {
  if (!frameId) return null;
  const real = globalThis.__telemaco_frameObjects?.[frameId]?.window;
  const existing = globalThis.__telemaco_frameWindows[frameId];
  if (!real) return existing || null;
  if (existing && existing.__telemaco_wrapsRealm) return existing;
  ...
  const win = new Proxy(real, {
    get(target, prop) {
      ...
      if (prop === '__telemaco_wrapsRealm') return true;
      // Not `receiver`: an accessor on a real global must run with the global
      // itself as `this`, not with this proxy.
      return Reflect.get(target, prop);
    },
    has(target, prop) {
      return prop === '__telemaco_wrapsRealm' || Reflect.has(target, prop);
    },
  });
  globalThis.__telemaco_frameWindows[frameId] = win;
  return win;
}

// The host calls this inside the target realm.
globalThis.__telemaco_deliverMessage = function(dataJson, origin, sourceFrameId, targetOrigin) {
  ...
  const source = (globalThis.__telemaco_frameId !== 0
                  && sourceFrameId === globalThis.__telemaco_parentFrameId)
    ? globalThis.parent
  ...
  try {
    ...
    globalThis.dispatchEvent(globalThis.__telemaco_markTrusted(
...
// __telemaco_init, before any of the document's own scripts run: `parent ===
...
function _installFramingRelationships() {
  if (!globalThis.__telemaco_frameId) return; // the page really is the top
  for (const [name, frameId] of [
    ['parent', globalThis.__telemaco_parentFrameId],
...
Element.prototype.attachShadow = function attachShadow(opts) {
  ...
  if (!globalThis.__telemaco_shadowHostNames.has(_ln) && _ln.indexOf('-') === -1) {
...
URL.createObjectURL = function(blob) {
  if (blob) {
    const id = 'blob:telemaco/' + Math.random().toString(36).substring(2);
    ...
    // The telemaco Blob materializes _bytes in its constructor; fall back to
...
if (!globalThis.crypto.subtle) {
  ...
  globalThis.__telemaco_clone_hooks = globalThis.__telemaco_clone_hooks || {};
  ...
  globalThis.__telemaco_clone_hooks["CryptoKey"] = function (src, seen) {
...
if (typeof BroadcastChannel === 'undefined') {
  ...
  // Telemaco page is one origin-bound browsing context today, so every channel
...
globalThis.__telemaco_init = function() {
  // The host sets __telemaco_frameId on a frame realm before calling this.
  _realmFrameId = globalThis.__telemaco_frameId >>> 0;
  ...
  const sw = Number.isFinite(globalThis.__telemaco_screen_w) && globalThis.__telemaco_screen_w > 0
    ? globalThis.__telemaco_screen_w : scr[0];
  const sh = Number.isFinite(globalThis.__telemaco_screen_h) && globalThis.__telemaco_screen_h > 0
    ? globalThis.__telemaco_screen_h : scr[1];
  ...
  const vw = Number.isFinite(globalThis.__telemaco_viewport_w) && globalThis.__telemaco_viewport_w > 0
    ? globalThis.__telemaco_viewport_w : sw;
  const vh = Number.isFinite(globalThis.__telemaco_viewport_h) && globalThis.__telemaco_viewport_h > 0
    ? globalThis.__telemaco_viewport_h : sh - 80;
  _applyScreenSize(sw, sh, !!globalThis.__telemaco_screen_emulated);
  ...
  // standalone runtime has the same 1x default as Telemaco's render surface.
  globalThis.devicePixelRatio = 1;
  globalThis.innerWidth = vw; globalThis.innerHeight = vh;
  globalThis.outerWidth = sw; globalThis.outerHeight = sh - 40;

  var hwValues = globalThis.__telemaco_stealth ? [4, 6, 8, 12, 16] : [2, 4, 6, 8, 12, 16];
  globalThis.__telemaco_hw = hwValues[Math.floor(_fpRand(400) * hwValues.length)];
  var memValues = globalThis.__telemaco_stealth ? [4, 8] : [0.25, 0.5, 1, 2, 4, 8];
  globalThis.__telemaco_mem = memValues[Math.floor(_fpRand(401) * memValues.length)];
  ...
  // Hide internals (_*, telemaco, Telemaco). The set of keys is static at
  ...
  const toHide = globalThis.__telemaco_hide_list || [];
  for (let i = 0; i < toHide.length; i++) {
    try { Object.defineProperty(globalThis, toHide[i], { enumerable: false }); } catch(e) {}
  }
  delete globalThis.__telemaco_init;
};
...
// for __telemaco_init to consume on every subsequent page. The snapshot
...
globalThis.__telemaco_hide_list = Object.getOwnPropertyNames(globalThis).filter(k =>
  k.startsWith('_') || k.includes('telemaco') || k.includes('Telemaco')
);
...
// tamperedFunctions: telemaco reimplements much of the DOM/Web platform in JS.
