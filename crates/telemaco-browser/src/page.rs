...
use telemaco_dom::{parse_html, DomTree};
use telemaco_js::frame::FrameRealm;
use telemaco_js::runtime::TelemacoJsRuntime;
use telemaco_net::{
    CallbackRegistry, TelemacoHttpClient, TelemacoNetError, RequestCallback, ResourceRequest,
...
/// Parse `TELEMACO_GEOLOCATION="lat,lon"` for the navigator.geolocation shim.
...
fn env_geolocation() -> Option<(f64, f64)> {
    let raw = std::env::var("TELEMACO_GEOLOCATION").ok()?;
...
pub struct Page {
    ...
    pub js: Option<TelemacoJsRuntime>,
    pub lifecycle: LifecycleState,
    pub http_client: Arc<TelemacoHttpClient>,
    ...
    intercept_tx: Option<tokio::sync::mpsc::UnboundedSender<telemaco_js::ops::InterceptedRequest>>,
...
fn max_live_frames() -> usize {
    std::env::var("TELEMACO_MAX_LIVE_FRAMES")
        .ok()
...
fn default_navigation_timeout() -> std::time::Duration {
    navigation_timeout_from_env_value(std::env::var("TELEMACO_NAV_TIMEOUT_MS").ok().as_deref())
}
...
fn default_navigation_chain_limit() -> usize {
    navigation_chain_limit_from_env_value(std::env::var("TELEMACO_NAV_CHAIN_LIMIT").ok().as_deref())
}

fn navigation_chain_limit_from_env_value(value: Option<&str>) -> usize {
    // Only an unreadable value falls back to the default, so
    // `TELEMACO_NAV_CHAIN_LIMIT=0` and `set_navigation_chain_limit(0)` agree.
    value
...
/// the layer this warms, `telemaco-render`: `font_face_declaration` ends in
...
/// `telemaco-render` applies before it will even consider one.
        ...
        r#"(function() {{
            ...
            function syncSheet() {{
                if (!style) {{
                    ...
                    style.setAttribute('data-telemaco-external-stylesheets', '');
                    style.textContent = `{escaped_css}`;
                    globalThis.__telemaco_registerLinkedStylesheet(link, style);
                }}
    ...
    format!(
        r#"(function() {{
            ...
            for (var i = 0; i < styles.length; i++) {{
                ...
                if (candidate.hasAttribute('data-telemaco-external-stylesheets')
                    || candidate.hasAttribute('data-telemaco-inline-import')) continue;
                authorIndex++;
            ...
            imported.setAttribute('data-telemaco-inline-import', '');
...
/// index excludes Telemaco's own materialized sheets so it remains stable while
...
fn inline_stylesheet_import_requests(dom: &DomTree) -> Vec<(usize, StylesheetImport)> {
    ...
    for style_id in style_ids {
        ...
        if node
            .get_attribute("data-telemaco-external-stylesheets")
            .is_some()
            || node.get_attribute("data-telemaco-inline-import").is_some()
        {
            continue;
        }
...
impl Page {
    pub fn new(id: String, context: Arc<BrowserContext>) -> Self {
        ...
        let stealth_client = if context.stealth {
            ...
            // (see telemaco-cli) rather than silently rewriting socks5:// to
    ...
    /// value takes precedence over `TELEMACO_NAV_TIMEOUT_MS`; callers that do
    ...
    /// Takes precedence over `TELEMACO_NAV_CHAIN_LIMIT`. A limit of 0 is
    ...
    fn deliver_frame_messages(&mut self) -> bool {
        ...
        for message in pending {
            ...
            if message.target_frame_id == 0 {
                ...
                let script = format!(
                    "globalThis.__telemaco_deliverMessage({escaped_data}, {escaped_origin}, {}, {escaped_target_origin});",
    ...
    fn forget_frame_references(&mut self, frame_id: u32, parent_frame_id: u32) {
        let script = format!(
            "if (globalThis.__telemaco_frameElements[{frame_id}] &&\
             globalThis.__telemaco_frameElements[{frame_id}]._frameId === {frame_id}) {{\
               globalThis.__telemaco_frameElements[{frame_id}]._frameId = 0;\
               if (globalThis.__telemaco_frameElements[{frame_id}]._iframeWin)\
                 globalThis.__telemaco_frameElements[{frame_id}]._iframeWin._frameId = 0;\
             }}\
             delete globalThis.__telemaco_frameObjects[{frame_id}];\
             delete globalThis.__telemaco_frameWindows[{frame_id}];\
             delete globalThis.__telemaco_frameElements[{frame_id}];"
        );
    ...
    fn release_detached_frames(&mut self) {
        ...
        const LIVE_FRAME_IDS: &str = "__telemaco_liveFrameIds()";
    ...
    /// in Telemaco's headless-only model.
    ...
    async fn do_fetch(&self, url: &Url) -> Result<Response, TelemacoNetError> {
        #[cfg(feature = "stealth")]
    ...
    fn init_js(&mut self) {
        ...
        let mut rt = TelemacoJsRuntime::with_base_url_and_proxy(
            &self.url_string(),
        ...
        if self.stealth_client.is_some() {
            rt.set_stealth(true);
            rt.set_user_agent(telemaco_net::STEALTH_USER_AGENT);
            rt.set_platform(
                telemaco_net::STEALTH_NAVIGATOR_PLATFORM,
                telemaco_net::STEALTH_UA_PLATFORM,
                telemaco_net::STEALTH_UA_PLATFORM_VERSION,
            );
        } else {
    ...
    async fn fetch_stylesheets(&mut self) -> Vec<(AuthorStylesheetTarget, String)> {
        ...
        while !pending.is_empty() {
            ...
            for (key, requested_url, depth, result) in results {
                ...
                let css = telemaco_net::decode_non_html(&response.body, response.content_type());
    ...
    async fn drive_load_delaying_scripts(
        ...
        while js.has_pending_load_delaying_scripts() {
            ...
            .await
            {
                ...
                Ok(Err(error)) => {
                    if telemaco_js::runtime::is_fatal_event_loop_error(&error) {
    ...
    async fn execute_scripts_with_module_budget(&mut self, module_budget_override: Option<u64>) {
        ...
        // synchronous hang. Raise it further with TELEMACO_SCRIPT_DEADLINE_MS=<ms>
        // for very heavy SPAs on slow networks (pair it with a matching client
        // navigation timeout).
        let script_deadline_ms: u64 = std::env::var("TELEMACO_SCRIPT_DEADLINE_MS")
            .ok()
            ...
            .map(|(idx, url)| {
                ...
                async move {
                    ...
                    if parsed.scheme() == "data" {
                        ...
                        let resp = telemaco_net::Response {
                            url: parsed,
                            status: 200,
                            headers,
                            body,
        ...
        let mut fetched: std::collections::HashMap<usize, (String, String, telemaco_net::Response)> =
            std::collections::HashMap::new();
        for result in fetch_results {
            if let Some((idx, url, resp)) = result {
                ...
                let code = telemaco_net::decode_non_html(&resp.body, resp.content_type());
        ...
        let module_budget_ms: u64 = {
            ...
            let short_ms: u64 = module_budget_override.unwrap_or_else(|| {
                std::env::var("TELEMACO_MODULE_BUDGET_MS")
                    .ok()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(3_000)
            });
        ...
        let module_hostcall_grace_ms = if module_budget_override.is_some() {
            ...
            std::env::var("TELEMACO_MODULE_HOSTCALL_GRACE_MS")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(5_000)
        };
        ...
        enum ScheduledScript {
            Classic(usize),
            Module {
                prepared: telemaco_js::runtime::PreparedModule,
                url: Option<String>,
        ...
        let execute_classic =
            |page: &mut Self,
             script: &ScriptInfo,
             fetched_script: Option<(String, String, telemaco_net::Response)>| {
    ...
    pub async fn navigate_with_wait_post(
        ...
        // with `TELEMACO_NAV_TIMEOUT_MS=NN`, or set a page-scoped deadline when
    ...
    pub async fn settle(&mut self, max_ms: u64) {
        ...
        loop {
            ...
            if let Some(js) = &mut self.js {
                if std::env::var_os("TELEMACO_STRICT_SETTLE").is_some() {
        ...
        #[cfg(feature = "render")]
        {
            ...
            let warmup_ms = std::env::var("TELEMACO_RENDER_RESOURCE_SETTLE_WARMUP_MS")
                .ok()
    ...
    async fn settle_runtime_for_duration(js: &mut TelemacoJsRuntime, duration_ms: u64) {
    ...
    async fn navigate_single(
        ...
        let (body_text, encoding_name) =
            telemaco_net::decode_response_with_name(&response.body, response.content_type());
        ...
        // waitUntil, so handlers that read window.__telemaco_css see it.
        if !author_stylesheets.is_empty() {
            if let Some(js) = &mut self.js {
                ...
                let code = format!("globalThis.__telemaco_css = `{}`;", escaped);
        ...
        #[cfg(feature = "render")]
        {
            let warmup_ms = std::env::var("TELEMACO_RENDER_RESOURCE_WARMUP_MS")
                .ok()
                .and_then(|value| value.parse::<u64>().ok())
                .unwrap_or(1_000);
        ...
        #[cfg(feature = "render")]
        {
            ...
            let warmup_ms = std::env::var("TELEMACO_RENDER_RESOURCE_POST_SCRIPT_WARMUP_MS")
                .ok()
                .and_then(|value| value.parse::<u64>().ok())
                .unwrap_or(1_000);
            let _ = self.prepare_screenshot_resources(warmup_ms).await;
    ...
    pub async fn prepare_screenshot_resources(&mut self, max_ms: u64) -> usize {
        ...
        let requested: Vec<(String, Option<telemaco_js::ImageRequestProfile>, ResourceType)> =
            candidates
                .into_iter()
        ...
        let requests = futures::stream::iter(requested.into_iter().map(|(raw, profile, kind)| {
            ...
            async move {
                ...
                match profile {
                    Some(telemaco_js::ImageRequestProfile::CorsSameOrigin) => {
                        request.mode = telemaco_net::RequestMode::Cors;
                        request.credentials = telemaco_net::RequestCredentials::SameOrigin;
                    }
                    Some(telemaco_js::ImageRequestProfile::CorsInclude) => {
                        request.mode = telemaco_net::RequestMode::Cors;
                        request.credentials = telemaco_net::RequestCredentials::Include;
                    }
                    _ => {}
                }
                #[cfg(feature = "stealth")]
    ...
    pub fn screenshot_at_animation_time(
        &self,
        viewport: (f32, f32),
        animation_sample_time: telemaco_js::AnimationSampleTime,
    ) -> Option<Vec<u8>> {
        self.screenshot_with_animation_sample(
            viewport,
            telemaco_js::AnimationSample {
                time: animation_sample_time,
                mode: telemaco_js::AnimationSampleMode::LocalOverride,
            },
        )
    }
    ...
    pub fn screenshot_with_animation_sample(
        &self,
        viewport: (f32, f32),
        animation_sample: telemaco_js::AnimationSample,
    ) -> Option<Vec<u8>> {
        ...
        self.with_dom(|dom| {
            telemaco_js::screenshot_png_scrolled_at_animation_time_with_surface_color(
                dom,
                viewport,
                base_url,
                scroll,
    ...
    pub fn screenshot_region(
        &self,
        region: telemaco_js::CaptureRegion,
    ) -> Result<Vec<u8>, telemaco_js::CaptureError> {
    ...
    pub fn screenshot_region_with_animation_sample(
        ...
        let js = self
            .js
            .as_ref()
            .ok_or(telemaco_js::CaptureError::PaintFailed)?;
        if !js.set_animation_sample(animation_sample) {
            return Err(telemaco_js::CaptureError::PaintFailed);
        }
    ...
    pub fn prepared_content_size_at_animation_time(
        ...
        self.prepared_content_size_with_animation_sample(telemaco_js::AnimationSample {
            time: animation_sample_time,
            mode: telemaco_js::AnimationSampleMode::LocalOverride,
        })
    }
    ...
    pub fn isolate_handle(&self) -> Option<telemaco_js::runtime::IsolateHandle> {
    ...
    pub async fn evaluate_for_cdp(
        ...
        await_promise: bool,
    ) -> telemaco_js::runtime::RemoteObjectInfo {
        if let Some(js) = &mut self.js {
            match js
    ...
    ) -> Result<telemaco_js::runtime::RemoteObjectInfo, String> {
        if let Some(js) = &mut self.js {
            ...
            Ok(telemaco_js::runtime::RemoteObjectInfo {
                js_type: match &value {
                    serde_json::Value::String(_) => "string".into(),
                    serde_json::Value::Number(_) => "number".into(),
                    serde_json::Value::Bool(_) => "boolean".into(),
    ...
    /// cached (e.g. it exceeded TELEMACO_NETWORK_BODY_BUFFER_BYTES and was
    ...
    pub fn take_pending_runtime_events(&mut self) -> Vec<telemaco_js::ops::RuntimeEvent> {
        if let Some(js) = &mut self.js {
    ...
    /// server wires the channel (`telemaco-cdp/src/server.rs`).
    pub fn enable_interception(
        &mut self,
    ) -> tokio::sync::mpsc::UnboundedReceiver<telemaco_js::ops::InterceptedRequest> {
        let (tx, rx) =
            tokio::sync::mpsc::unbounded_channel::<telemaco_js::ops::InterceptedRequest>();
    ...
    pub fn set_intercept_tx(
        &mut self,
        tx: tokio::sync::mpsc::UnboundedSender<telemaco_js::ops::InterceptedRequest>,
    ) {
...
mod tests {
    ...
    fn navigation_chain_limit_reads_the_environment_variable_by_name() {
        std::env::set_var("TELEMACO_NAV_CHAIN_LIMIT", "17");
    ...
    /// running the suite with `TELEMACO_NAV_CHAIN_LIMIT` set would see these
    ...
    async fn linked_stylesheet_graph_fetches_once_and_preserves_order_and_bases() {
        ...
        page.on_request(std::sync::Arc::new(move |request| {
            if request.resource_type == telemaco_net::ResourceType::Stylesheet {
        ...
        let sheets = page
            ...
            .unwrap()
            .with_dom(|dom| {
                dom.query_selector_all("style[data-telemaco-external-stylesheets]")
                    .unwrap()
                    .into_iter()
        ...
        for path in ["/a.css", "/b.css"] {
            assert_eq!(
                observed_requests
                    ...
                    .collect::<Vec<_>>(),
                vec![telemaco_net::ResourceType::Stylesheet],
            ...
            for path in ["/local.svg", "/imported.svg"] {
                assert_eq!(
                    observed_requests
                        ...
                        .map(|(_, resource_type)| *resource_type)
                        .collect::<Vec<_>>(),
                    vec![telemaco_net::ResourceType::Image],
    ...
    async fn a_frame_inside_a_shadow_root_survives_the_detach_sweep() {
        ...
        let published = page
            ...
            .as_mut()
            .unwrap()
            .evaluate("Object.keys(globalThis.__telemaco_frameObjects).length")
            .unwrap();
    ...
    async fn removing_an_iframe_releases_its_realm() {
        ...
        let left = page
            ...
            .unwrap()
            .evaluate(
                "Object.keys(globalThis.__telemaco_frameObjects).length\
                 + Object.keys(globalThis.__telemaco_frameWindows).length\
                 + Object.keys(globalThis.__telemaco_frameElements).length",
            )
            .unwrap();
        assert_eq!(
            left.as_f64(),
            ...
            page.js
                ...
                .evaluate(
                    "[document.readyState, globalThis.__postLoadDynamicRan === true, \
                     globalThis.__telemaco_hasPendingDynamicScripts(), \
                     globalThis.__telemaco_hasPendingLoadDelayingScripts()]",
                )
                .unwrap(),
    ...
    async fn page_transport_prefetches_once_and_capture_reuses_the_bytes() {
        ...
        let mut runtime = telemaco_js::runtime::TelemacoJsRuntime::new();
        runtime.set_dom(dom);
        ...
        let state = runtime
            .with_dom(|dom| {
                let link = dom
                    ...
                    .expect("async link");
                let styles = dom
                    .query_selector_all("style[data-telemaco-external-stylesheets]")
                    .expect("valid selector");
                (
                    dom.get_node(link)
        ...
        let state = runtime
            .with_dom(|dom| {
                ...
                (
                    dom.get_node(link)
                        .and_then(|node| node.get_attribute("data-loaded").map(str::to_owned)),
                    dom.query_selector("style[data-telemaco-external-stylesheets]")
                        .expect("valid selector")
                        .and_then(|style| {
                            dom.get_node(style)
            ...
            .evaluate(
                ...
                (() => {
                    ...
                    const source = document.querySelector(
                        'style[data-telemaco-external-stylesheets]'
                    );
                    return {
...
pub enum PageError {
    ...
    /// telemaco-net, and report `TelemacoNetError::TooManyRedirects`.
    #[error("Too many client-initiated navigations, the chain reached its limit of {0} documents")]
    TooManyClientNavigations(usize),
}

impl From<TelemacoNetError> for PageError {
    fn from(e: TelemacoNetError) -> Self {
        PageError::NetworkError(e.to_string())
    }
}
