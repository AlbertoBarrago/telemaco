...
///   TELEMACO_PROFILE=<index>   pin a specific profile from PROFILES
///   TELEMACO_ROTATE_PROFILE=1  pick a random profile per context
pub fn select_profile() -> &'static BrowserProfile {
    if let Some(idx) = std::env::var("TELEMACO_PROFILE")
        .ok()
        .as_deref()
        .map(str::trim)
    ...
    if env_enabled("TELEMACO_ROTATE_PROFILE") {
