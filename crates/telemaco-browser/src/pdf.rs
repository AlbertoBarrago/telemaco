...
fn validate_selected_raster_work(
    ...
    if !pixel_width.is_finite()
        || pixel_width <= 0.0
        || pixel_width > telemaco_js::MAX_CAPTURE_DIMENSION as f32
    {
        return Err(RasterPdfError::RasterWorkLimitExceeded);
    }
    ...
    for &page_index in selected_pages {
        ...
        if !slice_height.is_finite()
            || slice_height <= 0.0
            || slice_height > telemaco_js::MAX_CAPTURE_DIMENSION as f32
        {
            return Err(RasterPdfError::RasterWorkLimitExceeded);
        }
...
impl Page {
    ...
    pub fn raster_pdf_with_animation_sample(
        ...
        let previous_media = js.set_render_media(telemaco_js::CssMediaType::Print);
        let result = (|| {
...
mod tests {
    ...
    fn raster_pdf_repeats_fixed_content_and_advances_flow_on_every_selected_page() {
        ...
        let dom = telemaco_dom::parse_html(
