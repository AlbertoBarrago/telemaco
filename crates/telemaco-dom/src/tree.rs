...
impl DomTree {
    ...
    pub fn insert_before(&self, existing_id: NodeId, new_sibling_id: NodeId) {
        ...
        // loops forever and the test page hangs while telemaco burns RAM.
    ...
    pub fn descendants(&self, node_id: NodeId) -> Vec<NodeId> {
        ...
        while let Some(child_id) = first {
            ...
            if children_to_push.len() > inner.nodes.len() {
                eprintln!("telemaco: sibling-chain cap hit at node {} - cycle", node_id.index());
                break;
            }
        ...
        while let Some(current) = stack.pop() {
            ...
            if result.len() > inner.nodes.len() {
                eprintln!(
                    "telemaco: descendants() cap hit at node {} ({} nodes) - tree has a cycle",
                    node_id.index(),
                    inner.nodes.len()
                );
                break;
            ...
            while let Some(child_id) = child {
                children_to_push.push(child_id);
                if children_to_push.len() > inner.nodes.len() {
                    eprintln!("telemaco: sibling-chain cap hit at node {} - cycle", current.index());
                    break;
                }
...
fn collect_text_inner(inner: &DomTreeInner, node_id: NodeId, buf: &mut String) {
    ...
    while let Some(id) = stack.pop() {
        steps += 1;
        if steps > max_steps {
            eprintln!("telemaco: collect_text_inner cap hit - tree has a cycle");
            break;
        }
            ...
            _ => {
                ...
                while let Some(child_id) = child {
                    ...
                    if kids.len() > inner.nodes.len() {
                        eprintln!("telemaco: collect_text_inner sibling cap hit - cycle");
                        break;
                    }
                    child = inner.nodes.get(child_id.index())
                        .and_then(|n| n.as_ref())
