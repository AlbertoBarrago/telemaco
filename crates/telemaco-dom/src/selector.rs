...
pub struct TelemacoSelector;

impl parser::SelectorImpl for TelemacoSelector {
...
pub struct TelemacoSelectorParser;

impl<'i> parser::Parser<'i> for TelemacoSelectorParser {
    type Impl = TelemacoSelector;
...
thread_local! {
    static SELECTOR_CACHE: std::cell::RefCell<
        std::collections::HashMap<String, std::sync::Arc<SelectorList<TelemacoSelector>>>,
    > = std::cell::RefCell::new(std::collections::HashMap::with_capacity(64));
}
const SELECTOR_CACHE_CAP: usize = 256;

fn parse_selector_uncached(selector: &str) -> Result<SelectorList<TelemacoSelector>, String> {
    let mut parser_input = cssparser::ParserInput::new(selector);
    let mut parser = cssparser::Parser::new(&mut parser_input);
    SelectorList::parse(&TelemacoSelectorParser, &mut parser, ParseRelative::No)
        .map_err(|e| format!("Failed to parse selector '{}': {:?}", selector, e))
}

pub fn parse_selector(selector: &str) -> Result<SelectorList<TelemacoSelector>, String> {
...
pub struct CompiledSelector {
    sel: parser::Selector<TelemacoSelector>,
    specificity: u32,
    keys: SubjectKeys,
...
fn subject_keys(sel: &parser::Selector<TelemacoSelector>) -> Vec<SelectorKey> {
