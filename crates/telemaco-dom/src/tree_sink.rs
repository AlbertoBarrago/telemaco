...
pub struct TelemacoElemName<'a> {
    _ref: Ref<'a, ()>,
    name: *const QualName,
}

impl<'a> fmt::Debug for TelemacoElemName<'a> {
...
impl<'a> ElemName for TelemacoElemName<'a> {
...
impl TreeSink for DomTree {
    type Handle = NodeId;
    type Output = Self;
    type ElemName<'a> = TelemacoElemName<'a>;
    ...
    fn elem_name<'a>(&'a self, target: &'a NodeId) -> TelemacoElemName<'a> {
...
pub fn parse_fragment_with_context(html: &str, context_name: QualName) -> DomTree {
    ...
    let tree = DomTree::new();
    // Telemaco's fragment parser backs innerHTML in a scripting-enabled
