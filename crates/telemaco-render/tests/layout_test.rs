...
//! site-specific hacks that used to live in telemaco-render for this exact markup.

use telemaco_dom::tree_sink::parse_html;
use telemaco_render::{layout_dom, layout_dom_with_images};
...
fn find_by_text(
    tree: &telemaco_dom::tree::DomTree,
    layout: &telemaco_render::DomLayout,
    needle: &str,
) -> Option<(f32, f32)> {
    let mut best: Option<(f32, telemaco_render::Rect)> = None;
...
fn scrolled_ratio_sized_flex_clip_keeps_absolute_visual_visible() {
    ...
    let pixmap = telemaco_render::paint_dom_scrolled(&tree, (1080.0, 400.0), None, (0.0, 8300.0))
...
fn long_text_run_wraps_across_multiple_lines() {
    ...
    let mut div_rect: Option<telemaco_render::Rect> = None;
    for (id, rect) in &layout.rects {
...
fn non_replaced_inline_ignores_used_width_height_and_min_max_sizes() {
    ...
    assert_eq!(
        ...
        telemaco_render::Dimension::Percent(1.0),
...
fn ratio_only_inline_svg_sizes_inside_an_auto_grid_row() {
    ...
    assert!(
        ...
        "viewBox ratio must transfer the final grid width before Telemaco's device-pixel rounding: {logo:?}"
    );
    assert!(
...
fn nested_is_descendant_utility_blockifies_matching_lines() {
    ...
    assert_eq!(layout.styles[&one].display, telemaco_render::Display::Block);
    assert_eq!(layout.styles[&two].display, telemaco_render::Display::Block);
...
fn length_line_height_computes_before_inheritance() {
    ...
    assert_eq!(
        ...
        Some(telemaco_render::LineHeight::Px(80.0))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(telemaco_render::LineHeight::Ratio(2.0))
    );
}
...
fn contextual_right_inset_uses_root_font_tokens() {
    ...
    assert_eq!(
        ...
        Some(telemaco_render::Dimension::Px(56.0))
    );
...
fn text_indent_is_inherited_and_initial_resets_to_zero() {
    ...
    assert_eq!(
        ...
        Some(telemaco_render::Dimension::Px(40.0))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(telemaco_render::Dimension::Px(0.0))
    );
    assert_eq!(
...
fn tailwind_line_counter_fixture() -> telemaco_dom::tree::DomTree {
    parse_html(
        r#"
        <style>
...
fn generated_counter_text_reaches_the_positioned_pseudo_paint_path() {
    let tree = tailwind_line_counter_fixture();
    let pixmap =
        telemaco_render::paint_dom(&tree, (120.0, 80.0), None).expect("counter fixture paints");
...
fn positioned_counter_inside_nested_inline_code_uses_the_code_gutter() {
    ...
    let pixmap = telemaco_render::paint_dom(&tree, (260.0, 100.0), None)
...
fn positioned_generated_text_honors_text_alignment_inside_its_box() {
    ...
    let pixmap =
        telemaco_render::paint_dom(&tree, (80.0, 144.0), None).expect("aligned counters paint");
...
fn percentage_height_chain_under_auto_height_atomic_parent_stays_indefinite() {
    ...
    for name in ["floated", "inline", "float-icon", "inline-icon"] {
        assert_eq!(
            ...
            telemaco_render::Dimension::Auto,
...
fn display_inherit_transitively_preserves_contents_and_block_display() {
    ...
    assert_eq!(block_child.display, telemaco_render::Display::Block);
...
fn closed_details_content_does_not_paint() {
    ...
    let pixmap =
        telemaco_render::paint_dom(&tree, (40.0, 40.0), None).expect("details fixture paints");
