...
fn inline_child_ok(
    ...
    match &node.data {
        ...
        telemaco_dom::tree::NodeData::Element { .. } => {
            let Some(elem) = node.as_element() else {
                return true;
            };
...
mod tests {
    ...
    fn surface_cull_fixture() -> (TextEngine, usize) {
        let tree = telemaco_dom::parse_html("<p id='copy'>Visible text</p>");
    ...
    fn system_ui_resolves_in_stack_order_to_chromium_linux_face() {
        ...
        let tree = telemaco_dom::parse_html("<p id='copy'>Italic system UI</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
    ...
    fn ranged_face_shapes_at_its_font_database_weight() {
        let tree = telemaco_dom::parse_html("<p id='copy'>Variable family</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
        let mut style = LayoutStyle::default();
        style.display = Display::Block;
    ...
    fn static_text_keeps_the_zero_allocation_variation_path() {
        let tree = telemaco_dom::parse_html("<p id='copy'>ordinary text</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
        let mut style = LayoutStyle::default();
        style.display = Display::Block;
    ...
    fn keep_all_never_inserts_controls_inside_grapheme_clusters() {
        ...
        let tree = telemaco_dom::parse_html(&format!("<p id='copy'>{source}</p>"));
        let copy = tree.get_element_by_id("copy").unwrap();
    ...
    fn variable_font_fixture() -> Vec<u8> {
        let encoded: String = include_str!("../tests/fonts/telemaco-vf-test.woff2.b64")
            .chars()
    ...
    fn variable_font_multi_axis_shaping_preserves_space_advance() {
        let mut engine = TextEngine::new_with_web_fonts(&[WebFont {
            ...
            family: Some("Telemaco VF Test".into()),
            weight: Some((100, 900)),
            italic: Some(false),
        }]);
        let tree = telemaco_dom::parse_html("<p id='copy'>Tools and</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
        let mut style = LayoutStyle::default();
        style.display = Display::Block;
        style.font_family = Some("Telemaco VF Test".into());
    ...
    fn render_variable_fallback(
        ...
        let tree = telemaco_dom::parse_html("<p id='copy'>A</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
        let mut style = LayoutStyle::default();
        style.display = Display::Block;
    ...
    fn render_variable_weight(weight: u16) -> ((f32, f32), u64, Vec<u16>) {
        let mut engine = TextEngine::new_with_web_fonts(&[WebFont {
            data: variable_font_fixture(),
            family: Some("Telemaco VF Test".to_string()),
            weight: Some((100, 900)),
            italic: Some(false),
        }]);
        let tree = telemaco_dom::parse_html("<p id='copy'>MMMMMMMM</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
        let mut style = LayoutStyle::default();
        style.display = Display::Block;
        style.font_family = Some("Telemaco VF Test".to_string());
    ...
    fn shaped_text_preserves_authored_alpha_through_mask_rasterization() {
        let render_alpha = |alpha: u8, underline: bool| {
            let tree = telemaco_dom::parse_html("<p id='copy'>Alpha</p>");
            let copy = tree.get_element_by_id("copy").unwrap();
            let mut style = LayoutStyle::default();
            style.display = Display::Block;
            style.font_size = Some(32.0);
    ...
    fn variable_glyph_cache_keys_include_weight_axis() {
        ...
        base.font_family = Some("Telemaco VF Test".to_string());
    ...
    fn text_only_inline_block_keeps_an_internal_shaping_context() {
        let tree = telemaco_dom::parse_html("<span id='icon'>ligature_name</span>");
...
gamma</div>"#,
    ...
    fn text_wrap_balance_changes_line_grouping_without_changing_line_count() {
        let tree = telemaco_dom::parse_html("<h1 id='hero'>Welcome to Mozilla</h1>");
    ...
    fn emoji_font_is_loaded_only_for_emoji_documents() {
        ...
        let emoji_ids: std::collections::HashSet<_> = engine.loaded_families["noto color emoji"]
            ...
            .collect();
        let tree = telemaco_dom::parse_html("<p id='copy'>➕ 😀 🚀</p>");
        let copy = tree.get_element_by_id("copy").unwrap();
        let style = LayoutStyle {
            display: Display::Block,
            font_size: Some(48.0),
