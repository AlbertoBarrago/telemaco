...
use telemaco_dom::selector::{CompiledSelector, Matcher, SelectorKey};
...
/// path outside the anchor. Telemaco stores the smaller information needed by
...
fn eval_supports_condition(condition: &str) -> Option<bool> {
    ...
    if lower.starts_with("selector(") && condition.ends_with(')') {
        ...
        return Some(telemaco_dom::selector::parse_selector(inner.trim()).is_ok());
    }
...
mod tests {
    ...
    fn test_invalidation_map(css: &str) -> InvalidationMap {
        let tree = telemaco_dom::parse_html("<html><body></body></html>");
    ...
    fn cascade_layer_target(
        ...
        let tree = telemaco_dom::parse_html(r#"<div id="target" class="target"></div>"#);
    ...
    fn pseudo_candidate_bucket_does_not_replace_full_selector_matching() {
        let tree = telemaco_dom::parse_html(r#"<div id="target" class="alpha blocked"></div>"#);
        let target = tree.get_element_by_id("target").unwrap();
        let sheet = Stylesheet::parse(
            &tree,
            &[r#"
    ...
    fn generated_content_resolves_host_attributes_and_resets() {
        let tree =
            telemaco_dom::parse_html(r#"<button id="cta" data-label="Get Started"></button>"#);
    ...
    fn generated_content_parses_counter_items_and_styles() {
        let tree = telemaco_dom::parse_html(r#"<span id="line"></span>"#);
    ...
    fn sampled_animation_style(css: &str, sample_ms: f32, target_id: &str) -> LayoutStyle {
        let tree = telemaco_dom::parse_html(&format!(r#"<div id="{target_id}"></div>"#));
    ...
    fn waapi_samples_opacity_and_transform_without_rewriting_inline_cascade() {
        let tree = telemaco_dom::parse_html(r#"<div id="target" style="opacity:.2"></div>"#);
        let target = tree.get_element_by_id("target").unwrap();
        ...
        let important_tree = telemaco_dom::parse_html(
    ...
    fn mozilla_stagger_samples_only_the_delay_zero_frame() {
        ...
        let tree = telemaco_dom::parse_html(&html);
        let css = format!(
            r#"
                @keyframes wave {{
                    0%, 8.333% {{ opacity:1 }}
    ...
    fn registered_custom_properties_reuse_an_unchanged_parent_map() {
        let tree = telemaco_dom::parse_html(r#"<div id="target"></div>"#);
        let css = r#"
            @property --private {
                syntax:"<percentage>";
                inherits:false;
    ...
    fn registered_percentage_initial_value_keeps_radial_gradient_valid() {
        let tree = telemaco_dom::parse_html(r#"<div id="pulse"></div>"#);
        let css = r#"
    ...
    fn container_evaluator_honors_tailwind_threshold_and_cache() {
        let tree = telemaco_dom::parse_html(r#"<div id="container"><div id="target"></div></div>"#);
