...
use telemaco_dom::tree::{DomTree, NodeId};
use taffy::prelude::*;
...
/// `telemaco-render` compiles and gives reasonable geometry with its default
...
pub enum RetainedStyleMutation {
    ...
    /// A Web Animation sample changed. Telemaco's WAAPI surface currently
...
fn apply_presentational_hints(node: &telemaco_dom::tree::Node, style: &mut crate::LayoutStyle) {
...
fn cascade_node_style(
    ...
    matcher: &mut telemaco_dom::selector::Matcher,
    styles: &mut HashMap<NodeId, crate::LayoutStyle>,
...
fn cascade_walk(
    ...
    let mut subtree_matchers: Vec<telemaco_dom::selector::Matcher> = Vec::new();
    ...
    for invalidation in map.relational_invalidations() {
        ...
        for anchor in candidates.iter().copied() {
            ...
            if invalidation.unrepresentable_outer_path {
                if std::env::var_os("TELEMACO_RENDER_TIMING").is_some() {
                    eprintln!(
    ...
    let relevant_children = tree
        ...
        .filter(|child| {
            tree.get_node(*child).is_some_and(|node| match &node.data {
                telemaco_dom::tree::NodeData::Element { .. } => true,
                telemaco_dom::tree::NodeData::Text { contents } => !contents.is_empty(),
                _ => false,
            })
        })
        .count();
...
fn layout_dom_with_web_fonts_pass_limit_at_animation_time(
    ...
    let timing = std::env::var("TELEMACO_RENDER_TIMING").is_ok();
...
fn layout_dom_once(
    ...
    let quirks_mode = !tree.descendants(tree.document()).into_iter().any(|id| {
        tree.get_node(id).map_or(false, |node| {
            matches!(node.data, telemaco_dom::tree::NodeData::Doctype { .. })
        })
    });
    cascade_walk(
        tree,
    ...
    let needs_emoji_font = descendants.iter().any(|id| {
        tree.get_node(*id).is_some_and(|node| match &node.data {
            telemaco_dom::tree::NodeData::Text { contents } => {
...
fn has_inline_content(
    ...
    rendered_children(tree, id).into_iter().any(|cid| {
        ...
        match &node.data {
            telemaco_dom::tree::NodeData::Text { contents } => !contents.trim().is_empty(),
            _ => styles
                .get(&cid)
                .map(|s| {
...
fn build_any(
    ...
    let is_text = tree
        .get_node(id)
        .map(|n| matches!(n.data, telemaco_dom::tree::NodeData::Text { .. }))
        .unwrap_or(false);
    if is_text {
    ...
    while index < effective_children.len() {
        let is_text = match effective_children[index] {
            EffectiveGridChild::Dom(node) => tree.get_node(node).is_some_and(|node| {
                matches!(node.data, telemaco_dom::tree::NodeData::Text { .. })
            }),
...
fn build_text_words(
    ...
    let telemaco_dom::tree::NodeData::Text { contents } = &node.data else {
        return Vec::new();
    };
...
fn build_mixed_block(
    ...
    for &cid in &flat {
        ...
        let is_text = matches!(node.data, telemaco_dom::tree::NodeData::Text { .. });
...
fn inline_wraps_only_in_flow_blocks(
    ...
    for cid in rendered_children(tree, id) {
        ...
        if let telemaco_dom::tree::NodeData::Text { contents } = &node.data {
            if !contents.trim().is_empty() {
                return false;
            }
            continue;
...
fn can_use_native_float_band(
    ...
    // its source box visible. Other Telemaco BFC markers do not yet have a
...
mod tests {
    ...
    fn retained_tree_styles_match_forced_full_mutation_matrix() {
        ...
        tree.with_node_mut(text, |node| {
            if let telemaco_dom::tree::NodeData::Text { contents } = &mut node.data {
    ...
    fn retained_mixed_outer_has_dependencies_match_forced_full() {
        ...
        let host = tree.get_element_by_id("host").unwrap();
        let text = tree.new_node(telemaco_dom::tree::NodeData::Text {
            contents: String::new(),
        });
