#!/usr/bin/env node
// Live view for telemaco serve.
//
// Streams what a headless telemaco browser sees to any local browser tab,
...
async function ensureServe() {
  ...
  // try spawning telemaco directly (works in normal installs and in proot ubuntu)
  try {
    const child = spawn("telemaco", ["serve", "--port", String(cdpPort)], {
      stdio: "ignore",
      detached: true,
    });
    child.unref();
  } catch {}
  // fallback for Termux proot setups where telemaco lives inside ubuntu
  try {
    const child = spawn("proot-distro", ["login", "ubuntu", "--", "/usr/local/bin/telemaco", "serve", "--port", String(cdpPort)], {
      stdio: "ignore",
      detached: true,
    });
    child.unref();
...
<!doctype html>
<html>
<head><meta charset="utf-8"><title>Telemaco live view</title>
