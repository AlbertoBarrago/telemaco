#!/usr/bin/env python3
"""Capture Telemaco and Chromium concurrently with the same real settle delay.
...
Telemaco binary can be captured concurrently so regressions are compared
...
CANONICAL_TELEMACO_PROFILE = 0
CANONICAL_COLOR_SCHEME = "light"
# Telemaco currently models the default motion preference, not `reduce`.
...
def feature_probe_javascript(box_quad_unavailable_reason):
    ...
    return (
        ...
        "const telemacoStyleMirror=element.tagName==='STYLE'&&"
        "(element.hasAttribute('data-telemaco-external-stylesheets')||"
        "element.hasAttribute('data-telemaco-linked'));"
        "if(telemacoStyleMirror)continue;"
...
def scroll_eval_expression(scroll):
    ...
    return telemaco_state_eval_expression(scroll)
...
def telemaco_state_eval_expression(
    scroll,
    ...
    return (
        ...
        "'style[data-telemaco-external-stylesheets],style[data-telemaco-linked]'"
        ")):[];"
        ...
        + feature_probe_javascript(
            "Telemaco CLI capture does not expose node-scoped CDP "
...
def parse_telemaco_capture_report(stdout):
...
def parse_telemaco_scroll_report(stdout):
    state = parse_telemaco_capture_report(stdout)
    if state is None:
        return None
...
def telemaco_environment(width, height, animation_time_ms=None):
    env = dict(
        os.environ,
        TELEMACO_SHOT_W=str(width),
        TELEMACO_SHOT_H=str(height),
        ...
        TELEMACO_PROFILE=str(CANONICAL_TELEMACO_PROFILE),
        ...
        TELEMACO_SHOT_EVAL_AT_CAPTURE="1",
        ...
        TELEMACO_SHOT_RESOURCE_WARMUP="1",
    )
    if animation_time_ms is not None:
        env["TELEMACO_SHOT_ANIMATION_TIME_MS"] = str(animation_time_ms)
    return env
...
def with_controlled_scroll_environment(env, scroll):
    ...
    if scroll is not None:
        env["TELEMACO_SHOT_SCROLL_X"] = str(scroll[0])
        env["TELEMACO_SHOT_SCROLL_Y"] = str(scroll[1])
    return env


def probe_telemaco_identity(binary):
    expression = (
...
def probe_telemaco_css_media(binary):
    ...
    with tempfile.TemporaryDirectory(prefix="telemaco-media-probe-") as directory:
...
def capture_telemaco(
    ...
    env = with_controlled_scroll_environment(
        telemaco_environment(width, height, animation_time_ms), scroll
    )
    command = [
        binary,
        "fetch",
    ...
    state_expression = telemaco_state_eval_expression(
        None,
        geometry_selectors,
    ...
    try:
        ...
        state = (
            parse_telemaco_capture_report(result.stdout)
        ...
        if state is not None:
            ...
            state["media"] = media
        scroll_state = (
            parse_telemaco_scroll_report(result.stdout)
...
def chromium_identity_override(session):
    """Keep request headers and navigator identity aligned with Telemaco."""
...
def attach_chromium_box_quads(session, state):
    ...
    models = {}
    object_group = "telemaco-parity-feature-probes"
    try:
...
def warm_chromium_capture(page):
    ...
    # Yield one bounded browser task turn, matching the Telemaco CLI warm-up.
...
def compare_page_states(telemaco, chromium):
    """Return explicit same-page and geometry deltas; never infer a parity verdict."""
    telemaco_document = (telemaco or {}).get("document") or {}
    chromium_document = (chromium or {}).get("document") or {}
    telemaco_geometry = (telemaco or {}).get("geometry") or {}
    ...
    return {
        "url_equal": (telemaco or {}).get("url") == (chromium or {}).get("url"),
        "ready_state_equal": (
            telemaco_document.get("ready_state")
            == chromium_document.get("ready_state")
        ),
        "element_count_delta": delta(
            telemaco_document.get("element_count"),
            chromium_document.get("element_count"),
        ),
        "outer_html_utf16_delta": delta(
            telemaco_document.get("outer_html_utf16"),
            chromium_document.get("outer_html_utf16"),
        ),
        "body_text_utf16_delta": delta(
            telemaco_document.get("body_text_utf16"),
            chromium_document.get("body_text_utf16"),
        ),
        "outer_html_fingerprint_equal": (
            telemaco_document.get("outer_html_fnv1a32") is not None
            and telemaco_document.get("outer_html_fnv1a32")
            == chromium_document.get("outer_html_fnv1a32")
        ),
        "normalized_outer_html_utf16_delta": delta(
            telemaco_document.get("normalized_outer_html_utf16"),
            chromium_document.get("normalized_outer_html_utf16"),
        ),
        "normalized_outer_html_fingerprint_equal": (
            telemaco_document.get("normalized_outer_html_fnv1a32") is not None
            and telemaco_document.get("normalized_outer_html_fnv1a32")
            == chromium_document.get("normalized_outer_html_fnv1a32")
        ),
        "body_text_fingerprint_equal": (
            telemaco_document.get("body_text_fnv1a32") is not None
            and telemaco_document.get("body_text_fnv1a32")
            == chromium_document.get("body_text_fnv1a32")
        ),
        "geometry_delta": {
            field: delta(
                telemaco_geometry.get(field),
...
def classify_state_comparability(
    telemaco, chromium, chromium_capture_boundary=None
):
    ...
    """
    telemaco = telemaco or {}
    chromium = chromium or {}
    telemaco_document = telemaco.get("document") or {}
    ...
    telemaco_probes = telemaco.get("geometry_probes") or []
    chromium_probes = chromium.get("geometry_probes") or []
    probe_pairs = []
    for index in range(min(len(telemaco_probes), len(chromium_probes))):
        left_probe = telemaco_probes[index] or {}
    ...
    telemaco_probe_total = sum(pair["telemaco"] for pair in probe_pairs)
    chromium_probe_total = sum(pair["chromium"] for pair in probe_pairs)
    maximum_probe_total = max(telemaco_probe_total, chromium_probe_total)
    ...
    telemaco_boundary_stable = telemaco.get(
    ...
    boundary_instability = (
        telemaco_boundary_stable is False or chromium_boundary_stable is False
    )
    url_mismatch = (
        bool(telemaco.get("url"))
        and bool(chromium.get("url"))
        and telemaco.get("url") != chromium.get("url")
    )
    gross_signals = [
        name
        for name, signal in (
    ...
    return {
        ...
        "evidence": {
            "capture_boundary": {
                "telemaco_shared": telemaco_boundary_stable,
...
def split_generated_id_variance(telemaco_id, chromium_id):
    """Describe one structured ID mismatch without deciding it is volatile."""
    if not telemaco_id or not chromium_id or telemaco_id == chromium_id:
        return None

    prefix_length = 0
    prefix_limit = min(len(telemaco_id), len(chromium_id))
    while (
        prefix_length < prefix_limit
        and telemaco_id[prefix_length] == chromium_id[prefix_length]
    ):
        prefix_length += 1

    suffix_length = 0
    suffix_limit = min(
        len(telemaco_id) - prefix_length,
        len(chromium_id) - prefix_length,
    )
    while (
        suffix_length < suffix_limit
        and telemaco_id[-1 - suffix_length] == chromium_id[-1 - suffix_length]
    ):
        suffix_length += 1
    ...
    if suffix_length:
        raw_suffix = telemaco_id[len(telemaco_id) - suffix_length :]
    ...
    suffix_start_telemaco = len(telemaco_id) - suffix_length
    suffix_start_chromium = len(chromium_id) - suffix_length
    prefix = telemaco_id[:prefix_length]
    suffix = telemaco_id[suffix_start_telemaco:] if suffix_length else ""
    telemaco_salt = telemaco_id[prefix_length:suffix_start_telemaco]
    chromium_salt = chromium_id[prefix_length:suffix_start_chromium]
    if not telemaco_salt or not chromium_salt:
        return None
    ...
    salt_is_generated = (
        ...
        and any(character.isdigit() for character in telemaco_salt)
        and any(character.isdigit() for character in chromium_salt)
        and len(telemaco_salt) <= 32
        and len(chromium_salt) <= 32
    )
    return {
        "prefix": prefix,
        "suffix": suffix,
        "telemaco_salt": telemaco_salt,
...
def compare_geometry_dom_ids(telemaco_nodes, chromium_nodes):
    """Compare IDs, allowing only repeated framework-generated salt changes."""
    if len(telemaco_nodes) != len(chromium_nodes):
        return {
            "comparable": False,
            "mismatches": [],
    ...
    for index, (telemaco_node, chromium_node) in enumerate(
        zip(telemaco_nodes, chromium_nodes)
    ):
        telemaco_id = (telemaco_node or {}).get("id") or ""
        chromium_id = (chromium_node or {}).get("id") or ""
        if telemaco_id == chromium_id:
            continue
        variance = split_generated_id_variance(telemaco_id, chromium_id)
        mismatch = {
            "node_index": index,
            "telemaco_id": telemaco_id,
        ...
        if variance and variance["generated_salt_candidate"]:
            mapping = (variance["telemaco_salt"], variance["chromium_salt"])
...
def compare_geometry_dom_structures(telemaco_dom, chromium_dom):
    ...
    truncated = available and (
        telemaco_structure.get("subtree_truncated") is True
...
def compare_geometry_probes(telemaco, chromium):
    """Report raw deltas and gate geometry verdicts on target structure."""
    telemaco_probes = (telemaco or {}).get("geometry_probes") or []
    chromium_probes = (chromium or {}).get("geometry_probes") or []
    comparisons = []
    for index in range(max(len(telemaco_probes), len(chromium_probes))):
        telemaco_probe = (
            telemaco_probes[index] if index < len(telemaco_probes) else None
        )
        chromium_probe = (
            chromium_probes[index] if index < len(chromium_probes) else None
        )
        telemaco_rects = (telemaco_probe or {}).get("rects") or []
        chromium_rects = (chromium_probe or {}).get("rects") or []
        rect_deltas = []
        subtree_comparisons = []
        for rect_index in range(min(len(telemaco_rects), len(chromium_rects))):
            telemaco_rect = telemaco_rects[rect_index]
            chromium_rect = chromium_rects[rect_index]
            subtree_comparison = compare_geometry_dom_structures(
                telemaco_rect.get("dom"), chromium_rect.get("dom")
            )
            subtree_comparisons.append(subtree_comparison)
            deltas = {}
            for field in ("x", "y", "width", "height"):
                left = telemaco_rect.get(field)
            ...
            telemaco_computed = telemaco_rect.get("computed") or {}
            chromium_computed = chromium_rect.get("computed") or {}
            computed_differences = {
                field: {
                    "telemaco": telemaco_computed.get(field),
                    "chromium": chromium_computed.get(field),
                }
                for field in sorted(
                    set(telemaco_computed) | set(chromium_computed)
                )
                if telemaco_computed.get(field) != chromium_computed.get(field)
            }
            rect_deltas.append(
                {
                    "index": rect_index,
                    "delta": deltas,
                    "visibility": {
                        "telemaco": telemaco_rect.get("visible"),
        ...
        telemaco_count = (telemaco_probe or {}).get("count")
        chromium_count = (chromium_probe or {}).get("count")
        selector_left = (telemaco_probe or {}).get("selector")
        selector_right = (chromium_probe or {}).get("selector")
        query_valid = (
            (telemaco_probe or {}).get("valid") is True
            and (chromium_probe or {}).get("valid") is True
        )
        counts_equal = (
            isinstance(telemaco_count, int)
            and not isinstance(telemaco_count, bool)
            and isinstance(chromium_count, int)
            and not isinstance(chromium_count, bool)
            and telemaco_count == chromium_count
        )
        verdict_exclusions = []
        if not query_valid:
        ...
        elif telemaco_count == 0:
            verdict_exclusions.append("no-matched-targets")
        if counts_equal and isinstance(telemaco_count, int) and telemaco_count > 0:
            expected_rects = min(
                telemaco_count,
                (telemaco_probe or {}).get("rect_limit")
        ...
        comparisons.append(
            {
                "index": index,
                "selector": (
                    (telemaco_probe or {}).get("selector")
                    if telemaco_probe is not None
                    else (chromium_probe or {}).get("selector")
                ),
                "valid": {
                    "telemaco": (telemaco_probe or {}).get("valid"),
                ...
                "errors": {
                    "telemaco": (telemaco_probe or {}).get("error"),
                    "chromium": (chromium_probe or {}).get("error"),
                },
                "counts": {
                    "telemaco": telemaco_count,
                    "chromium": chromium_count,
                    "delta": (
                        telemaco_count - chromium_count
                        if isinstance(telemaco_count, int)
                        and isinstance(chromium_count, int)
                        else None
                    ),
                },
...
def compare_feature_probes(telemaco, chromium):
    """Compare selector-free candidates by category and document element index."""
    telemaco_features = (telemaco or {}).get("feature_probes") or {}
    chromium_features = (chromium or {}).get("feature_probes") or {}
    telemaco_categories = telemaco_features.get("categories") or {}
    ...
    for kind in sorted(set(telemaco_categories) | set(chromium_categories)):
        telemaco_category = telemaco_categories.get(kind) or {}
        chromium_category = chromium_categories.get(kind) or {}
        telemaco_candidates = {
            candidate.get("comparison_index", candidate.get("dom_index")): candidate
            for candidate in telemaco_category.get("candidates") or []
            if isinstance(
        ...
        for comparison_index in sorted(
            set(telemaco_candidates) | set(chromium_candidates)
        ):
            telemaco_candidate = telemaco_candidates.get(comparison_index)
            chromium_candidate = chromium_candidates.get(comparison_index)
            telemaco_computed = (telemaco_candidate or {}).get("computed") or {}
            ...
            computed_differences = {
                ...
                for field in sorted(set(telemaco_computed) | set(chromium_computed))
                if telemaco_computed.get(field) != chromium_computed.get(field)
            }
            telemaco_quads = (telemaco_candidate or {}).get("box_quads") or {}
            chromium_quads = (chromium_candidate or {}).get("box_quads") or {}
            quad_spaces_equal = (
                telemaco_quads.get("coordinate_space") is not None
                and telemaco_quads.get("coordinate_space")
            ...
            candidate_comparisons.append(
                {
                    ...
                    "dom_index": {
                        "telemaco": (telemaco_candidate or {}).get("dom_index"),
                        "chromium": (chromium_candidate or {}).get("dom_index"),
                    },
                    "present": {
                        "telemaco": telemaco_candidate is not None,
                        "chromium": chromium_candidate is not None,
                    },
                    "candidate_reasons": {
                        "telemaco": (telemaco_candidate or {}).get(
                            "candidate_reasons"
                        ),
        ...
        telemaco_seen = telemaco_category.get("matches_seen")
        chromium_seen = chromium_category.get("matches_seen")
        category_comparisons.append(
            {
                "kind": kind,
                "matches_seen": {
                    "telemaco": telemaco_seen,
                    "chromium": chromium_seen,
                    "delta": numeric_delta(telemaco_seen, chromium_seen),
                },
                "candidates_truncated": {
                    "telemaco": telemaco_category.get("candidates_truncated"),
    ...
    return {
        "bounds": {
            "telemaco": telemaco_features.get("bounds"),
            "chromium": chromium_features.get("bounds"),
        },
        "scan": {
            "telemaco_scanned_elements": telemaco_features.get("scanned_elements"),
            "chromium_scanned_elements": chromium_features.get("scanned_elements"),
            "telemaco_comparable_scanned_elements": telemaco_features.get(
                "comparable_scanned_elements"
            ),
            "chromium_comparable_scanned_elements": chromium_features.get(
                "comparable_scanned_elements"
            ),
            "telemaco_truncated": telemaco_features.get("scan_truncated"),
            "chromium_truncated": chromium_features.get("scan_truncated"),
        },
        "box_quad_capabilities": {
            "telemaco": telemaco_features.get("box_quads"),
...
def main():
    ...
    parser.add_argument("--telemaco-bin", required=True)
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "matches Telemaco's deterministic static animation sample"
        ),
    )
    parser.add_argument(
        "--geometry-selector",
    ...
    if args.settle_ms % 1000:
        parser.error(
            "--settle-ms must be a whole number of seconds because Telemaco's "
    ...
    manifest = {
        ...
        "animation_sampling": (
            {
                ...
                "telemaco": "static renderer animation sample time T=0",
            }
        ...
        "telemaco": binary_version(args.telemaco_bin),
        ...
        "capture_identity": {
            ...
            "telemaco_profile": CANONICAL_TELEMACO_PROFILE,
        },
        "capture_media": {
            "normalized": True,
            "color_scheme": CANONICAL_COLOR_SCHEME,
            ...
            "reduced_motion_reason": (
                "Telemaco currently models the browser default "
        ...
        "methodology_limits": {
            ...
            "page_state": (
                ...
                "fingerprints exclude only Telemaco's explicitly marked external-"
            ...
            "resource_readiness": (
                ...
                "Telemaco's pre-paint state with Chromium's post-load state. "
            ...
            "automatic_feature_probes": (
                ...
                "the adjacent DOM/body-text provenance evidence. Telemaco's marked "
                ...
                "scroll metrics are intentionally excluded because Telemaco's "
                "current CLI surface cannot yet provide trustworthy values."
            ),
            "box_quads": (
                "Chromium content and border quads come from CDP "
                "DOM.getBoxModel in viewport CSS pixels. Telemaco CLI capture "
    ...
    manifest["telemaco_identity_probe"] = probe_telemaco_identity(args.telemaco_bin)
    manifest["telemaco_css_media_probe"] = probe_telemaco_css_media(args.telemaco_bin)
    if args.baseline_bin:
        manifest["baseline_identity_probe"] = probe_telemaco_identity(args.baseline_bin)
        manifest["baseline_css_media_probe"] = probe_telemaco_css_media(
            args.baseline_bin
        )
    results_path = out / "results.json"
    write_results(results_path, manifest)
    probes = [
        ("telemaco JS media", manifest["telemaco_identity_probe"].get("media_matches_configured")),
        ("telemaco CSS media", manifest["telemaco_css_media_probe"].get("ok")),
    ]
    if args.baseline_bin:
        probes.extend(
            [
    ...
    with sync_playwright() as playwright:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            for index, url in enumerate(urls):
                ...
                ours_path = out / f"{name}.telemaco.png"
                ...
                context.close()
                page_result["telemaco"] = ours_future.result()
                if baseline_future:
                    page_result["baseline"] = baseline_future.result()

                if chrome_ok and page_result["telemaco"]["ok"]:
                    page_result["page_state_comparison"] = compare_page_states(
                        page_result["telemaco"].get("state"),
                        chromium_state,
                    )
                ...
                if (
                    ...
                    and chrome_ok
                    and page_result["telemaco"]["ok"]
                ):
                    ours_scroll = page_result["telemaco"].get("scroll_state") or {}
    ...
    failed = [
        page["name"]
        for page in manifest["pages"]
        if not page.get("chromium", {}).get("ok") or not page.get("telemaco", {}).get("ok")
    ]
    if failed:
