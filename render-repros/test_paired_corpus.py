...
class ControlledScrollTests(unittest.TestCase):
    ...
    def test_capture_report_uses_post_settle_state(self):
        ...
        state = paired_corpus.parse_telemaco_scroll_report(stdout)
    ...
    def test_telemaco_capture_exports_final_scroll_request_to_cli(self):
        environment = paired_corpus.telemaco_environment(1280, 720)
        paired_corpus.with_controlled_scroll_environment(
            environment, (12, "bottom")
        )
        self.assertEqual(environment["TELEMACO_SHOT_SCROLL_X"], "12")
        self.assertEqual(environment["TELEMACO_SHOT_SCROLL_Y"], "bottom")
        self.assertEqual(environment["TELEMACO_SHOT_EVAL_AT_CAPTURE"], "1")
        self.assertEqual(environment["TELEMACO_SHOT_RESOURCE_WARMUP"], "1")
        self.assertNotIn("TELEMACO_SHOT_ANIMATION_TIME_MS", environment)

    def test_telemaco_capture_exports_explicit_animation_sample(self):
        environment = paired_corpus.telemaco_environment(1280, 720, 0)
        self.assertEqual(environment["TELEMACO_SHOT_ANIMATION_TIME_MS"], "0")

    def test_paired_state_expression_is_read_only_at_capture_boundary(self):
        expression = paired_corpus.telemaco_state_eval_expression(
            None,
            [".card"],
    ...
    def test_every_capture_expression_samples_page_state_without_scrolling(self):
        expression = paired_corpus.telemaco_state_eval_expression(None)
        self.assertIn("outer_html_fnv1a32", expression)
        self.assertIn("data-telemaco-external-stylesheets", expression)
    ...
    def test_capture_report_keeps_dom_state_and_authoritative_geometry(self):
        ...
        state = paired_corpus.parse_telemaco_capture_report(stdout)
    ...
    def test_page_state_comparison_reports_provenance_and_geometry_deltas(self):
        ...
        comparison = paired_corpus.compare_page_states(telemaco, chromium)
...
class StateComparabilityTests(unittest.TestCase):
    ...
    def test_capture_boundary_instability_excludes_otherwise_matching_state(self):
        telemaco = self.state(100, 800, [2, 4, 1])
    ...
    def test_one_moderate_provenance_difference_does_not_overreject(self):
        telemaco = self.state(70, 800, [2, 4, 1])
        chromium = self.state(100, 800, [2, 4, 1])
...
class GeometryProbeTests(unittest.TestCase):
    ...
    def test_default_state_expressions_do_not_include_probe_work(self):
        telemaco_expression = paired_corpus.telemaco_state_eval_expression(None)
        self.assertNotIn("sampleGeometrySelector", telemaco_expression)
        self.assertNotIn("geometry_probes", telemaco_expression)
        self.assertIn("feature_probes:featureProbes", telemaco_expression)
    ...
    def test_telemaco_records_quad_capability_instead_of_fabricated_corners(self):
        expression = paired_corpus.telemaco_state_eval_expression(None)
        self.assertIn(
    ...
    def test_repeatable_selectors_are_passed_safely_in_one_state_expression(self):
        ...
        telemaco_expression = paired_corpus.telemaco_state_eval_expression(
            (0, 25), selectors
        )
        encoded = json.dumps(selectors, ensure_ascii=True, separators=(",", ":"))
        self.assertIn(encoded, telemaco_expression)
        self.assertIn("sampleGeometrySelector", telemaco_expression)
        self.assertIn("catch(error)", telemaco_expression)
        self.assertIn("geometry_probes:geometryProbes", telemaco_expression)
    ...
    def test_probe_comparison_reports_raw_deltas_and_invalid_errors(self):
        ...
        comparison = paired_corpus.compare_geometry_probes(telemaco, chromium)
        ...
        self.assertEqual(
            ...
            {
                "align_items": {"telemaco": "stretch", "chromium": "normal"},
                "width": {"telemaco": "100px", "chromium": "98px"},
            },
        )
        self.assertEqual(comparison[1]["valid"], {"telemaco": False, "chromium": False})
    ...
    def test_geometry_target_accepts_repeated_angular_id_salt_variants(self):
        ...
        self.assertEqual(comparison["telemaco"]["nodes"][0]["id"], "ng-tab-7")
        self.assertEqual(
    ...
    def test_feature_comparison_reports_presence_computed_geometry_and_quad_capability(self):
        ...
        comparison = paired_corpus.compare_feature_probes(telemaco, chromium)
        ...
        self.assertFalse(first["box_quads"]["telemaco_available"])
        ...
        self.assertEqual(
            second["present"], {"telemaco": False, "chromium": True}
        )
